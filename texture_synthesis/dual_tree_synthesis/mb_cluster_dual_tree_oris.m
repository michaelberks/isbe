function mb_cluster_dual_tree_oris(varargin)
...
cluster_idx = get_cluster_idx(...
    args.DualTreeDir, args.Level, args.Orientation,...
...
% Get data for orientations {1,2,3} or {4,5,6}
data_args.Orientation = args.Orientation + 0; % = 1 or 4
[data1] =...
    mb_get_dual_tree_subband_data(data_args);

% Get data for orientations {1,2,3} or {4,5,6}
data_args.Orientation = args.Orientation + 1; % = 2 or 5
[data2] =...
    mb_get_dual_tree_subband_data(data_args);

% Get data for orientations {1,2,3} or {4,5,6}
data_args.Orientation = args.Orientation + 2; % = 3 or 6
[data3] =...
    mb_get_dual_tree_subband_data(data_args);

args.ClusteringFunctionArgs.Data = [data1 data2 data3];
...
function all_indices = get_cluster_idx(...
    DualTreeDir, Level, Orientation, WindowSize, MaxMemory, overlap, WindowSize2)
...
mem_per_sample = 3*(WindowSize^2 + WindowSize2^2) * size_of_data_element;
...
for ii = 1:number_of_trees
	...
	sampled = zeros(size(dual_tree{Level}(:,:,Orientation)));
