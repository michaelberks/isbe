function [frames_rgb] = load_hdf5_frames(hdf5_file, frames_path, frames_idx, rotate, save_dir)
...
if rotate
    frames_rgb = rot90(frames_rgb, rotate);
end
...
if ~isempty(save_dir)
    ...
    for i_fr = 1:length(frames_idx)
        fr = frames_idx(i_fr);
        imwrite(uint8(frames_rgb(:,:,:,fr)), [save_dir '\frame' zerostr(fr, 5) '.png']);
    end
end
