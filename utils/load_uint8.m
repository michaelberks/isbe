function [out_var, scaling] = load_uint8(filepath,varargin)
...
check_zero = false;
if nargin>1, check_zero = varargin{1}; end
...
if (check_zero)
	...
	out_var(abs(s.out_var-zero_val)<=1 & abs(zero_val-s.out_var)<=1) = 0;
end

if isfield(scaling, 'complex')
    dims = size(out_var);
