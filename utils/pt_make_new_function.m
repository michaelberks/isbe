function pt_make_new_function(name, use_packargs, args_in, args_out)
%PT_MAKE_NEW_FUNCTION creates a new function with documentation template
%   [] = pt_make_new_function(fcn_name,use_packargs,args_in,args_out)
%
%   inputs:
%      fcn_name - String giving the name of the new function
...
%     
%     >> pt_make_new_function('mymultiply',0,{'x','y'},{'z'});
%   
%   notes:
%
...
author = '%% Author: Phil Tresadern \n';
email = '%% Email : philip.tresadern@manchester.ac.uk \n';
phone = '%% Phone : +44 (0)161 275 5114 \n';
...
fprintf(fid,'\n');
fprintf(fid,'f_debug = (nargin == 0 && nargout == 0);\n');
fprintf(fid,'if f_debug, test_script(); return; end\n');
fprintf(fid,'\n');
funccaller = ['[' outputs '] = func(' inputs ');\n'];
fprintf(fid,'\n');
fprintf(fid,'\n');
fprintf(fid,'%% The function\n');
funcheader = ['function [' outputs '] = func(' inputs ')\n'];
fprintf(fid,'\n');
fprintf(fid,'\n');
fprintf(fid,'%% Test script\n');
fprintf(fid,'function test_script()\n');
fprintf(fid,'clc;\n');
funccaller = ['[' outputs '] = ' name '(' inputs ');\n'];
fprintf(fid,'\n');
