...
if nargout>=1, local_mean = zeros(size(image_in)); end
if nargout>=2, local_std = zeros(size(image_in)); end
...
for y = 1:size(image_in,1)
	for x = 1:size(image_in,2)
		...
		local_std(y,x) = var_scl*(sum_p2/n_pix - p_mean*p_mean);
	end
end
