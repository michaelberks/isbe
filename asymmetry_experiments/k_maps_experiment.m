...
map_type = {'g2d', 'rf_thin', 'rf_thresh', 'rf_prob', 'rf_mix', 'rf_uni', 'g2d_uni', 'rf_uni2', 'rf_mix2'};
...
for aa = 1:2 %Compute for both normals and normals
    ...
    for mm = 9 %For each method rf, wrf, g2
        if mm > 5
            spacing = 4;
        else
            spacing = 2;
...
f_type = {'f1', 'f2'};
for mm = 9, for ss=1:2, for ff=1:2, %#ok
...
map_type = {'g2d', 'rf_thin', 'rf_thresh', 'rf_prob', 'rf_mix'};
scale_type = {'_max_scale', '_template_scale'};
f_type = {'f1', 'f2'};
ab_type = {'normals', 'abnormals'};
...
f_type = {'f1', 'f2'};
ab_type = {'normals', 'abnormals'};
for mm = 9, for ss=1, for ff=1:2, for aa = 1:2 %#ok
...
for aa = 1:2, for mm = 9, for ss=1, for ff=1:2 %#ok
...
scale_type = {'_max_scale', '_template_scale'};
ab_type = {'normals', 'abnormals'};
for mm = 9, for ss=1, for aa = 1:2 %#ok
