function [tp fp fp_pixels] = k_maps_roc(map_dir, data_type, varargin)
...
f_map_list = dir([map_dir '/*' args.feature_type '*.mat']);
f_mask_list = dir([map_dir '/*mask.mat']);

mammo_names = get_mammo_info(f_map_list);
...
for ii = 1:num_mammos
    ...
    f_array = load_uint8([map_dir f_map_list(ii).name]);
    f_mask = u_load([map_dir f_mask_list(ii).name]);
    
    %Convert array into map and resize according to spacing
    f_map = zeros(size(f_mask));
    f_map(f_mask) = f_array;
    f_map = f_map(args.offset_y+(1:args.spacing:end),args.offset_x+(1:args.spacing:end));
    [r c] = size(f_map);
    
    %smooth map
    if args.sigma
        f_map = imfilter(f_map, fspecial('gaussian', 5*args.sigma+1, args.sigma));
    end
