...
foveal_list = dir([args.foveal_mask_dir '/*.mat']);
...
for kk = 1:num_images
    ...
    switch args.prediction_type
        case {'detection','logistic_classification','centre_detection'}
            ...
            foveal_mask = u_load([args.foveal_mask_dir foveal_list(this_im).name]);

            %Check we have enough samples in data
            total_v_pts = sum(vessel_mask(:) & foveal_mask(:));
            total_b_pts = sum(~vessel_mask(:) & foveal_mask(:));
        ...
        case {'foveal_detection', 'foveal_centre_detection'}
            %Load in vessel mask
            vessel_mask = u_load([args.vessel_mask_dir vessel_list(this_im).name]);

            if strcmp(args.prediction_type, 'foveal_centre_detection')
                %Create skeletonised vessel mask
                vessel_mask = bwmorph(vessel_mask, 'thin', 'inf');
            end
            ...
            edge_mask = foveal_mask & ~imerode(foveal_mask, strel('disk', 32));
