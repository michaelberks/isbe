function [training_data training_labels] = resample_retinogram_data(varargin)
...
sample_map_list = dir([args.sample_map_dir '/*.mat']);
vessel_mask_list = dir([args.vessel_mask_dir '/*.mat']);
...
for kk = 1:num_images
    ...
    switch args.prediction_type
        case {'detection','logistic_classification','centre_detection'}
            ...
            sample_map = load_uint8([args.sample_map_dir sample_map_list(this_im).name]);

            %Load in vessel mask
            vessel_mask = load_uint8([args.vessel_mask_dir vessel_mask_list(this_im).name]);
            
            %Check we have enough samples in data
            total_pts = sum(sample_map(:) > 0);
            num_samples_image = min(num_samples_image, total_pts); %just in case...
            display(['Samples in image ' num2str(kk) ': ', num2str(num_samples_image)]);

            %Get random sample pixels according to sampling weightings
            image_idx = sample_from_probs(sample_map(:), num_samples_image);
          
            %Save the sample labels for this image
            training_labels(curr_sample:num_samples_image+curr_sample-1) = ...
                vessel_mask(image_idx);
            
            if args.debug
                [y x] = ind2sub(size(vessel_mask), image_idx);
                figure; 
                subplot(1,2,1); imgray(sample_map); plot(x, y, 'r.');
                subplot(1,2,2); imgray(vessel_mask); plot(x, y, 'r.');
            end
