function [ center, radii, e_axes, v, chi2 ] = ellipsoid_fit( X, equals )
%
...
[center, radii, e_axes, eigen_signs] = ellipsoid_params( v );
...
d = d * e_axes; % rotate to cardinal axes of the conic;
d = [ d(:,1) / radii(1), d(:,2) / radii(2), d(:,3) / radii(3) ]; % normalize to the conic radii
chi2 = sum( abs( 1 - sum( d.^2 .* repmat( eigen_signs', size( d, 1 ), 1 ), 2 ) ) );
