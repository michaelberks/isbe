function [] = fit_ellipsoid_gui(raw_volumes, varargin)
%FIT_ELLIPSOID_GUI *Insert a one line summary here*
%   [] = fit_ellipsoid_gui(varargin)
%
...
args = u_packargs(varargin, 0, ...
    ...
    'radii_steps', [0.1 0.1 0.1],...
    'center_steps', [0.1 0.1 0.1],...
    'rotation_steps', [pi/100 pi/100 pi/100],...
    'feature_fun', 'none',...
    'grad_sigma', 1,...
    'x_roi', [],...
    'y_roi', [],...
...
%--------------------------------------------------------------------------
    function create_main_fig
        ...
        ui.menu_item_load_seq = uimenu(ui.menu,...
            'Label', 'Load new sequence',...
            'Callback', @load_new_sequence);
        ui.menu_item_load_mask = uimenu(ui.menu,...
            'Label','Load mask',...
            'Callback', @load_mask);
        ui.menu_item_make_video = uimenu(ui.menu,...
            'Label','Make video',...
            'Callback', @make_video);
        ui.menu_item_quit = uimenu(ui.menu,...
            'Label','Quit',...
        ...
        ui.center_z_inc = uicontrol(... 
            'Position', [2*buff+button_w panel_h-(5*buff+6*text_h) button_w text_h],...
            'String', '+',...
            'Style', 'pushbutton',...
            'Parent', ui.panel_center,...
            'Enable', 'off',...
            'Callback', @center_z_inc);
        
        ui.center_z_dec = uicontrol(... 
            'Position', [buff panel_h-(5*buff+6*text_h) button_w text_h],...
            'String', '-',...
            'Style', 'pushbutton',...
            'Parent', ui.panel_center,...
            'Enable', 'off',...
            'Callback', @center_z_dec);

        ui.show_ellipse = uicontrol(... 
            'Position', [3*buff+2*button_w panel_h-(buff+2*text_h) button_w text_h],...
            'String', 'Show ellipse',...
            'Style', 'checkbox',...
            'Parent', ui.panel_center,...
            'Enable', 'on',...
        ...
        ui.show_roi = uicontrol(... 
            'Position', [3*buff+2*button_w panel_h-(2*buff+3*text_h) button_w text_h],...
            'String', 'Show ROI boundary',...
            'Style', 'checkbox',...
            'Parent', ui.panel_center,...
            'Enable', 'on',...
            'Value', 1,...
        ...
        ui.radii_z_inc = uicontrol(... 
            'Position', [2*buff+button_w panel_h-(5*buff+6*text_h) button_w text_h],...
            'String', '+',...
            'Style', 'pushbutton',...
            'Parent', ui.panel_radii,...
            'Enable', 'off',...
            'Callback', @radii_z_inc);
        
        ui.radii_z_dec = uicontrol(... 
            'Position', [buff panel_h-(5*buff+6*text_h) button_w text_h],...
            'String', '-',...
            'Style', 'pushbutton',...
            'Parent', ui.panel_radii,...
            'Enable', 'off',...
            'Callback', @radii_z_dec);
        ...
        ui.rot_z_text = uicontrol(... 
            'Parent', ui.panel_axes,... 
            'Style','text',...
            'BackgroundColor', get(ui.panel, 'BackgroundColor'),...
            'FontName', 'Arial',...
            'String', 'Rotate around z-axis:',...
            'HorizontalAlignment', 'left',...
            'Position', [buff panel_h-(text_h+buff) text_w text_h]); 
        
        ui.rot_z_inc = uicontrol(... 
            'Position', [2*buff+button_w panel_h-(buff+2*text_h) button_w text_h],...
            'String', '+',...
            'Style', 'pushbutton',...
            'Parent', ui.panel_axes,...
        ...
        ui.rot_y_text = uicontrol(... 
            'Parent', ui.panel_axes,... 
            'Style','text',...
            'BackgroundColor', get(ui.panel, 'BackgroundColor'),...
            'FontName', 'Arial',...
            'String', 'Rotate around y-axis',...
            'HorizontalAlignment', 'left',...
            'Position', [buff panel_h-(3*buff+3*text_h) text_w+button_w+buff text_h]); 
        
        ui.rot_y_inc = uicontrol(... 
            'Position', [2*buff+button_w panel_h-(3*buff+4*text_h) button_w text_h],...
            'String', '+',...
            'Style', 'pushbutton',...
            'Parent', ui.panel_axes,...
...
%--------------------------------------------------------------------------
    ...
    function load_volumes()
        ...
        if ~isempty(args.mask) &&...
            ...
            e3_axes = eye(3);
            e3_center = [mean(args.x(1,:,1)) mean(args.y(:,1,1)) mean(args.z(1,1,:))];
            e3_radii = [std(args.x(1,:,1)) std(args.y(:,1,1)) std(args.z(1,1,:))];
        end
        curr_vol = 1;
        load_volume();
        enable_controls();
    ...
    function make_ellipse_from_mask(mask)
        ...
        [~, surface_points] = ...
            make_tumour_surface(mask, [], args.voxel_dims, args.x, args.y, args.z);
        [e3_center, e3_radii, e3_axes, e3_params] = ellipsoid_fit(surface_points);

    end

    function update_ellipse()
        e3_params = ellipsoid_general(e3_center, e3_radii, e3_axes);
        [e3_weighted_volume, e3_volume] = weight_ellipsoid_volume(e3_params, args.x, args.y, args.z, e3_weight);
        if ~e3_weight
            e3_weight = 1 / mean(e3_volume(e3_volume > 0));
        end
        old_match_score = match_score;
        match_score = sum(feature_volume(:) .* e3_weighted_volume(:));
        update_view();
    end
    ...
    function update_view()
        ...
        set(ui.ellipse(1), 'visible', 'off');
        if get(ui.show_ellipse, 'value')
            [v2] = compute_ellipsoid_z_slice(e3_params, args.z(1,1,curr_slice));
        ...
        set(ui.tumour1, 'visible', 'off');
        if get(ui.show_roi, 'value') && ...
                ~isempty(surface_points) && any(surface_points(:,3)==args.z(1,1,curr_slice))
        ...
        set(ui.ellipse(2), 'visible', 'off');
        if get(ui.show_ellipse, 'value')    
            %Display the current ellipse in the slice
            [v2] = compute_ellipsoid_x_slice(e3_params, args.x(1,curr_col,1));
            [e2_center, e2_radii, e2_axis, e2_signs] = ellipse_params( v2 );
            if all(e2_signs(:) > 0)
                [ey2, ez2] = ...
                    ellipse(e2_radii(1), e2_radii(2), e2_center(1), e2_center(2), e2_axis(:,1), 0, 50);

                eyi2 = interp1(squeeze(args.y(:,1,1)), linspace(1, axes_pos2(3), num_rows)', ey2);
                ezi2 = interp1(squeeze(args.z(1,1,:)), linspace(1, axes_pos2(4), num_slices)', ez2);
                set(ui.ellipse(2),...
                    'XData', eyi2,...
                    'YData', ezi2,...
                    'visible', 'on');
        ...
        set(ui.tumour2,'visible', 'off');
        if get(ui.show_roi, 'value') && ...
                ~isempty(surface_points) && any(surface_points(:,1)==args.x(1,curr_col,1))
           tidx = surface_points(:,1)==args.x(1,curr_col,1);
           tyi = interp1(squeeze(args.y(:,1,1)), linspace(1, axes_pos2(3), num_rows), surface_points(tidx,2));
           tzi = interp1(squeeze(args.z(1,1,:)), linspace(1, axes_pos2(4), num_slices), surface_points(tidx,3));
        ...
        %Display the current ellipse in the slice
        set(ui.ellipse(3), 'visible', 'off');
        if get(ui.show_ellipse, 'value')
            [v2] = compute_ellipsoid_y_slice(e3_params, args.y(curr_row,1,1));
            [e2_center, e2_radii, e2_axis, e2_signs] = ellipse_params( v2 );
            if all(e2_signs(:) > 0)
                [ex2, ez2] = ...
                    ellipse(e2_radii(1), e2_radii(2), e2_center(1), e2_center(2), e2_axis(:,1), 0, 50);
                ...
                ezi2 = interp1(squeeze(args.z(1,1,:)), linspace(1, axes_pos3(4), num_slices)', ez2);
                set(ui.ellipse(3),...
                    'XData', exi2,...
                    'YData', ezi2,...
                    'visible', 'on');
        ...
        set(ui.tumour3,'visible', 'off');
        if get(ui.show_roi, 'value') && ...
                ~isempty(surface_points) && any(surface_points(:,2)==args.y(curr_row,1,1))
           tidx = surface_points(:,2)==args.y(curr_row,1,1);
           txi = interp1(args.x(1,:,1), linspace(1, axes_pos3(3), num_cols), surface_points(tidx,1));
           tzi = interp1(squeeze(args.z(1,1,:)), linspace(1, axes_pos3(4), num_slices), surface_points(tidx,3));
    ...
    function update_control_display()
        set(ui.center_x_text, 'String',...
            ['X center:' num2str(e3_center(1),4)]);
        set(ui.center_y_text, 'String',...
            ['Y center:' num2str(e3_center(2),4)]);
        set(ui.center_z_text, 'String',...
            ['Z center:' num2str(e3_center(3),4)]);
        set(ui.radii_x_text, 'String',...
            ['X radius:' num2str(e3_radii(1),4)]);
        set(ui.radii_y_text, 'String',...
            ['Y radius:' num2str(e3_radii(2),4)]);
        set(ui.radii_z_text, 'String',...
            ['Z radius:' num2str(e3_radii(3),4)]);
...
% --------------------------------------------------------------------
    ...
    % Callback...        
    	e3_center(1) = e3_center(1) + args.center_steps(1);        
        update_ellipse();
...
% --------------------------------------------------------------------
    ...
    % Callback...
    	...
    	e3_center(2) = e3_center(2) - args.center_steps(2);
        update_ellipse();
...
% --------------------------------------------------------------------
    function center_z_inc(hObject, eventdata) %#ok
    % Callback...        
    	e3_center(3) = e3_center(3) + args.center_steps(3);
        update_ellipse();

    end

% --------------------------------------------------------------------
    function center_z_dec(hObject, eventdata) %#ok
    % Callback...       
    	e3_center(3) = e3_center(3) - args.center_steps(3);
        update_ellipse();
...
% --------------------------------------------------------------------
    ...
    % Callback...        
    	e3_radii(1) = e3_radii(1) + args.radii_steps(1);
        update_ellipse();
...
% --------------------------------------------------------------------
    ...
    % Callback...
        if (e3_radii(1) - args.radii_steps(1)) > 0
            e3_radii(1) = e3_radii(1) - args.radii_steps(1);
            update_ellipse();
        end
...
% --------------------------------------------------------------------
    ...
    % Callback...
    	...
    	e3_radii(2) = e3_radii(2) + args.radii_steps(2);
        update_ellipse();
...
% --------------------------------------------------------------------
    ...
    % Callback...
        if (e3_radii(2) - args.radii_steps(2)) > 0
            e3_radii(2) = e3_radii(2) - args.radii_steps(2);
            update_ellipse();
        end
...
% --------------------------------------------------------------------
    function radii_z_inc(hObject, eventdata) %#ok
    % Callback...
    	e3_radii(3) = e3_radii(3) + args.radii_steps(3);
        update_ellipse();

    end

% --------------------------------------------------------------------
    function radii_z_dec(hObject, eventdata) %#ok
    % Callback...
        if (e3_radii(3) - args.radii_steps(3)) > 0
            e3_radii(3) = e3_radii(3) - args.radii_steps(3);
            update_ellipse();
        end
...
% --------------------------------------------------------------------
    function rot_z_inc(hObject, eventdata) %#ok
    % Callback...
        %Rotate about z-axis
        R = make_3d_rotation(args.rotation_steps(3), [0 0 1]');
        e3_axes = R*e3_axes;
    	update_ellipse();
...
% --------------------------------------------------------------------
    function rot_z_dec(hObject, eventdata) %#ok
    % Callback...
        %Rotate about y-axis
        R = make_3d_rotation(-args.rotation_steps(3), [0 0 1]');
        e3_axes = R*e3_axes;
...
% --------------------------------------------------------------------
    function rot_y_inc(hObject, eventdata) %#ok
    % Callback...
        %Rotate about z-axis
        %Rotate about y-axis
        R = make_3d_rotation(args.rotation_steps(2), [0 1 0]');
        e3_axes = R*e3_axes;
    	update_ellipse();
...
% --------------------------------------------------------------------
    function rot_y_dec(hObject, eventdata) %#ok
    % Callback...
        %Rotate about z-axis
        %Rotate about y-axis
        R = make_3d_rotation(-args.rotation_steps(2), [0 1 0]');
...
% --------------------------------------------------------------------
    function rot_x_inc(hObject, eventdata) %#ok
    % Callback...
        %Rotate about z-axis
        %Rotate about y-axis
        R = make_3d_rotation(args.rotation_steps(1), [1 0 0]');
        e3_axes = R*e3_axes;
    	update_ellipse();
...
%---------------------------------------------------------------------
    function show_callback(hObject, eventdata) %#ok
       update_view(); 
    end
...
% --------------------------------------------------------------------
    function make_video(hObject, eventdata) %#ok
    % Callback... 
        [gif_name, gif_path] = ...
            uiputfile('*.gif', 'Select output video', args.root_dir);
        
        if ~gif_name
            return;
        end
    	curr_vol = 1;
        gif_name = fullfile(gif_path, gif_name);
        for i_vol = 1:num_vols
            curr_vol = i_vol;
            load_volume();
            frame1 = getframe(ui.main_fig);
            gif1 = frame2im(frame1);
            [gif1a, map] = rgb2ind(gif1, 2^12);
            if curr_vol == 1
