function [snake_pnts,e] = mb_snake_normal_closed(pnts, alpha, beta, max_delta_x, resol_x, ...
...
% Copyright (c) 1995 Interval Research Corporation.
  ...
  for k = 2:n-1,
    ...
    for v2 = 1:num_states, 
        for v1 = 1:num_states,
            ...
            continuity = ...
                + alpha * ( ...
                    (pos_x(k,v1)-pos_x(k-1,v0_domain)).^2 ...
                +   (pos_y(k,v1)-pos_y(k-1,v0_domain)).^2) ...
                ...
                + beta * (...
  ...
  for v1 = 1:num_states,
      ...
      %Work back through the snake to find the 1st point associated with
      %each penultimate state path
      v1a = v1;
      v0 = zeros(1,num_states);
      for v2 = 1:num_states
          v2a = v2;
          for k = n-2:-1:1
      
            v = Imat(k+1,(v2a-1)*num_states+v1a);
            v2a = v1a; 
            v1a = v;            
          end;
          v0(v2) = v2a;
      ...
      %Use this to compute the beta energy of the final points
      
      %Workout the optimal path to the final point
    v0_domain = 1:num_states;
    [y,i] = min( Smat(n-1,(v1-1)*num_states+v0_domain) ...
        + alpha * ( ...
            (pos_x(n,v1)-pos_x(n-1,v0_domain)).^2 + ...
            (pos_y(n,v1)-pos_y(n-1,v0_domain)).^2) ...
        + beta * (...
                (pos_x(1,v0)-2*pos_x(n,v1) + pos_x(n-1,v0_domain)).^2 ...
            +   (pos_y(1,v0)-2*pos_y(n,v1) + pos_y(n-1,v0_domain)).^2));
