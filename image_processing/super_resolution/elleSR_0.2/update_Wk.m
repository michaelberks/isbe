function [W_k] = update_Wk(ni_hi, nj_hi, ni_lo, nj_lo, theta_k, gamma_k, mask)
...
nzmax = ceil(pi*16*gamma_k*( theta_k(5)*theta_k(1) - theta_k(4)*theta_k(2) ))*ni_lo*nj_lo;
...
thresh = stdlim*stdlim*gamma_k;

for j_lo = 1:nj_lo %(i_h_lo=0; i_h_lo < h_lo[kj]; i_h_lo++) { % iterate over low-res image's rows. */
    for i_lo = 1:ni_lo %(i_v_lo=0; i_v_lo < v_lo[kj]; i_v_lo++) { % scan down each column in turn. */
        ...
        if (j_c >=0 && j_c<nj_hi && i_c>=0 && i_c<ni_hi) % this means the mapped low-res pixel lands somewhere in the superimage. */
            ...
            if (norm_val > 0.0000001) % if all the values are low, I'll flag the pixel as unsupported rather than assume good values. */
                for j_hi = j0:j1 % iterate back across the patch, storing the normalized values in W. */
                    for i_hi = i0:i1
                        if h(i_hi, j_hi)>0
                            %Col index for this pixel in hi-es image X
                            W_cidx(wpt) = (j_hi-1)*ni_hi+i_hi;

                            %Store column as computed above
                            W_ridx(wpt) = W_ci;
...
%This is sloooww... Don't bother. Remove any indices and values we didn't use
% W_ridx(wpt:end) = [];
% W_cidx(wpt:end) = [];
% W_vals(wpt:end) = [];

%Build the sparse system matrix W from the indices and values
W_k = sparse(W_ridx(1:wpt-1), W_cidx(1:wpt-1), W_vals(1:wpt-1), ni_lo*nj_lo, ni_hi*nj_hi);
