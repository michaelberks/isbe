...
void DistanceTransform3D(const InputType* pIn, OutputType* D, const int* dims)
{
...
{
	if( nrhs != 1 ) 
		mexErrMsgTxt("Syntax: D = ac_distance_transform_3d_dll(BW)");
	if( mxGetNumberOfDimensions(prhs[0]) != 3 )
		mexErrMsgTxt("The input matrix must be of 3D!"); 
	...
	DistanceTransform3D(pI, pOut, dims); 
}