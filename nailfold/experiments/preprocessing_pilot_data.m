...
apex_dir = [pilot_dir 'apexes\'];
%--------------------------------------------------------------------------
%% Make mask enhanced copies of the images
mkdir([pilot_dir 'images_enhanced']);

for i_im = 1:num_images
    
    %load in image
    im_num = pilot_images(i_im).name(1:end-4);   
    nailfold = imread([pilot_dir 'images\' im_num '.png']);
    ...
    write_im_from_colormap(nailfold, [pilot_dir 'images_enhanced\' im_num '.png'], gray(256), clims); 
...
%     imwrite(nailfold, [pilot_dir 'images_enhanced\' im_num '.png']);
...
mkdir([pilot_dir 'apex_clusters\'])
for i_im = 1:num_images%1:
  
    %load in image
    im_num = pilot_images(i_im).name(1:end-4);       
    [vessels] = cluster_vessel_apices(im_num, [pilot_dir 'markup\'], markers, 20, 1);
    save([pilot_dir 'apex_clusters\' im_num '_apex_clusters.mat'], 'vessels');
end
...
save_apex_pts_txt = 0;
save_apex_pts_mat = 0;
save_vessel_pts_txt = 0;
save_vessel_pts_mat = 0;
save_vessel_patch = 1;
do_plot = 0;
num_v_pts = 15;
min_v_pts_sz = 30;
...
for i_im = 1:num_images
    ...
    %load in image
    im_num = pilot_images(i_im).name(1:end-4);   
    nailfold_name = [pilot_dir 'images\' im_num '.png'];   
    ...
    for i_ma = 1:num_markers
        ...
        %Check if marker has marked this image
        markup_list = dir([pilot_dir 'markup\' marker_name '\*#' im_num '*']);         
        if isempty(markup_list); continue; end
        
        %Read in vessel markup data - where multiple markup files exist,
        %use the last one
        markup = read_markup_from([pilot_dir 'markup\' marker_name '\' markup_list(end).name]);
...
%%
mkdir([pilot_dir '\models']);
save([pilot_dir '\models\apex_templates.mat'], 'mean_apex_*');
%%
load([pilot_dir '\models\apex_templates.mat'], 'mean_apex_*');

mkdir([pilot_dir '\results\template_matching\original\intensity']);
mkdir([pilot_dir '\results\template_matching\aligned\intensity']); 
mkdir([pilot_dir '\results\template_matching\original\g12d']);
mkdir([pilot_dir '\results\template_matching\aligned\g12d']);
...
for i_im = 2:num_images
    ...
    for i_sz = 1:3
        ...
        for i_sh = 1:3
            ...
            save([pilot_dir '\results\template_matching\original\g12d\' im_num ...
            ...
            save([pilot_dir '\results\template_matching\aligned\g12d\' im_num ...
                '_apex_candidates' num2str(i_sh) '_' num2str(i_sz) '.mat'],...
                'maxima_pos', 'maxima_vals');
            ...
            save([pilot_dir '\results\template_matching\original\intensity\' im_num ...
                '_apex_candidates' num2str(i_sh) '_' num2str(i_sz) '.mat'],...
                'maxima_pos', 'maxima_vals');
            ...
            save([pilot_dir '\results\template_matching\aligned\intensity\' im_num ...
                '_apex_candidates' num2str(i_sh) '_' num2str(i_sz) '.mat'],...
                'maxima_pos', 'maxima_vals');
...
for i_im = 1:2%num_images
    ...
    for i_sz = 1:3
        for i_sh = 1:3
            ...
            load([pilot_dir '\results\template_matching\original\' im_num ...
                '_apex_candidates' num2str(i_sh) '_' num2str(i_sz) '.mat'],...
                'maxima_pos', 'maxima_vals');
            ...
            load([pilot_dir '\results\template_matching\aligned\' im_num ...
                '_apex_candidates' num2str(i_sh) '_' num2str(i_sz) '.mat'],...
                'maxima_pos', 'maxima_vals');
            plot(maxima_pos(1:20, 1), maxima_pos(1:20, 2),...
                'o', 'markersize', 3*i_sz);
