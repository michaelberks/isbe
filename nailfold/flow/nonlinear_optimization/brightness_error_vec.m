function [res, p_test, Jrows,Jcols, Hrows,Hcols] = ...
    brightness_error_vec(var_vec, sz_vec, observations, ...
                         f_clear_persistent)
% Return the error vector corresponding to the brightness terms.
% If more than 
...
% Unpack the parameter vector into a structure that we can manage more
% easily.
variables = ncm_unpack(var_vec, sz_vec);
% Strip the parameters of interest from the (variables,obserations) pair.
...
% Define zero displacement for first frame
...
% Compute the actual error vector
...
% Define the nonzero elements of the Jacobian that correspond to the
% brightness terms.
% (This is a slightly tedious function but speeds up the optimization no
% end.)
