...
if isempty(args.selected_segments)
    ...
    stationary_mosaics_cropped = zeros(length(args.mosaic_lims_y), length(args.mosaic_lims_x), num_segs);
    stationary_masks = false(size(stationary_mosaics_cropped));
    stationary_mosaics = cell(num_segs,1);
    focused_idx = cell(num_segs, 1);
    segment_idx = cell(num_segs, 1);
    intra_segment_transforms = cell(num_segs,1);
    reject_segments = false(num_segs,1);
    stationary_mosaic_centres = zeros(num_segs, 2);
    matched_counts = cell(num_segs,1);
else
    load([args.sequence_dir 'segments\segment_data.mat'],...
        'stationary_mosaics_cropped', 'stationary_mosaics', 'focused_idx', 'intra_segment_transforms',...
        'reject_segments', 'stationary_mosaic_centres', 'matched_counts');%
end
...
for i_seg = args.selected_segments
    ...
    stationary_mosaic_centres(i_seg,1) = seg_x(round(num_frames/2));
    stationary_mosaic_centres(i_seg,2) = seg_y(round(num_frames/2));
   ...
   %Extract the largest consectutive sequence of focused frames   
    ...
    if good_frames > args.min_stationary_frames
        build_video = true;
        segment_idx{i_seg} = focused_idx_i;
        
    elseif good_frames >= target_frames
        build_video = false;
        frames = frames(:,:,~reject_frames);
        segment_idx{i_seg} = focused_idx_i(~reject_frames);
    ...
    initial_frames = round(linspace(1, size(frames,3), 10));
    [initial_transforms] = ...
        register_tiles_features(frames(:,:,initial_frames), ...
                            'theta_range', args.theta_range, ...
                            'offset_lim', args.intra_segment_offset, ...
                            'sigma', args.sigma,...
                            'tile_masks', tile_masks(:,:,initial_frames),...
                            'debug', false);
    [target_mosaic] = ...
            create_mosaic(frames(:,:,initial_frames), initial_transforms);                   
    
    [intra_segment_transforms{i_seg}, matched_counts{i_seg}] = ...
    ...
    if build_video
        [stationary_mosaics{i_seg}, ~, ~, cleaned_frames, edge_mask] = ...
            create_mosaic(frames, intra_segment_transforms{i_seg});
        
        reg_folder = [args.sequence_dir 'registered' zerostr(i_seg,2) '\'];
        
        create_folder(reg_folder)
        delete([reg_folder 'frame*']); %Just in case already soemthing there
        for i_f = 1:num_frames
            imwrite(uint8(255*cleaned_frames(:,:,i_f)), [reg_folder 'frame' zerostr(i_f,4) '.png']);
        end
        imwrite(uint8(255*~edge_mask), [reg_folder 'seq_mask.png']);
        ...
        [stationary_mosaics{i_seg}] = ...
            create_mosaic(frames, intra_segment_transforms{i_seg});
    end

    [r, c] = size(stationary_mosaics{i_seg});
    cx = round(c/2);
    cy = round(r/2);
    stationary_mosaics_cropped(:,:,i_seg) = stationary_mosaics{i_seg}(cy+args.mosaic_lims_y, cx+args.mosaic_lims_x);
    
    cx = round(args.frame_w/2);
    cy = round(args.frame_h/2);
    stationary_masks(:,:,i_seg) = dirt_mask(cy+args.mosaic_lims_y, cx+args.mosaic_lims_x);
...
progress = 2; %#ok
save([args.sequence_dir 'segments\segment_data.mat'],...
    'stationary_mosaics_cropped', 'stationary_mosaics', 'focused_idx', 'segment_idx', 'intra_segment_transforms',...
    'reject_segments', 'stationary_mosaic_centres', 'matched_counts', 'progress',  '-append');
...
if any(reject_segments)
    ...
    for i_r = find(reject_segments)
        ...
        is_connected = is_mosaic_connected(stationary_mosaic_centres(required_segments,:), frame_w, frame_h);
    ...
    %stationary_mosaics_cropped(:,:,reject_segments) = [];
    %stationary_mosaics(reject_segments) = []; %#ok
    %stationary_mosaic_centres(reject_segments,:) = [];
end

if size(stationary_mosaic_centres, 1) < 2
    return;
end

%Use motor positions to initialise alignment of frames
offset_centres = [diff(stationary_mosaic_centres(required_segments,:)*args.pixels_per_mm); 0 0];
offset_centres(:,2) = -offset_centres(:,2);

[inter_segment_transforms] = ...
        register_tiles_features(stationary_mosaics_cropped(:,:,required_segments), ...
                            'theta_range', args.theta_range, ...
                            'offset_lim', args.inter_segment_offset, ...
                            'offset_centres', offset_centres,...
                            'sigma', args.sigma,...
                            'tile_masks', stationary_masks,...
                            'debug', false);
[segment_mosaic, segment_mosaic_weights, inter_segment_transforms] = ...
    create_mosaic(stationary_mosaics_cropped, inter_segment_transforms, 'diamond'); %#ok
segment_mask = segment_mosaic_weights > 0.1; %#ok

progress = 3; %#ok
save([args.sequence_dir 'segments\segment_data.mat'],...
    'segment_mosaic', 'segment_mosaic_weights',...
    'inter_segment_transforms', 'segment_mask', 'required_segments', 'progress', '-append');
...
function is_connected = is_mosaic_connected(stationary_mosaic_centres, frame_w, frame_h)

num_segs = size(stationary_mosaic_centres,1);
x_diffs = zeros(num_segs);
y_diffs = zeros(num_segs);
for i_seg = 1:num_segs
    x_diffs(:,i_seg) = abs(stationary_mosaic_centres(:,1) - stationary_mosaic_centres(i_seg,1));
    y_diffs(:,i_seg) = abs(stationary_mosaic_centres(:,2) - stationary_mosaic_centres(i_seg,2));
end
