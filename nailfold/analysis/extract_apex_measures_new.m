function [apex_measures] = ...
    extract_apex_measures_new(vessel_prob, vessel_ori, vessel_width, vessel_im, apex_xy, varargin)
...
x_sq = repmat(linspace(-x_lim, x_lim, args.patch_sz(2)), args.patch_sz(2), 1);
y_sq = x_sq';
circle_mask = x_sq.^2 + y_sq.^2 > x_lim^2;
xy_sq = [x_sq(:) y_sq(:)];
...
apex_measures.initial_connected_orientation = zeros(num_apexes,1);
...
for i_pt = 1:num_apexes
    ...
    %candidate position
    xya = xy_sq * scale;
    xa = reshape(xya(:,1) + ax, args.patch_sz(2), args.patch_sz(2));
    ya = reshape(xya(:,2) + ay, args.patch_sz(2), args.patch_sz(2));
    ...
    vessel_prob_patch(circle_mask) = 0;
    ...
    %Now resample a path given a better estimate of vessel orientation
    a_theta = angle(-apex_measures.initial_connected_orientation(i_pt)) / 2;
    ...
    %measures
    vessel_ori_patch = vessel_ori_patch * conj(-apex_measures.initial_connected_orientation(i_pt));
