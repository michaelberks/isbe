...
x = repmat(patch_idx, args.patch_size, 1);
y = x';
circ_mask = false(args.patch_size,args.patch_size,4);
for i_sz = 1:4
    p_sz_i = pad_sz / 2^(i_sz-1);
    circ_mask(:,:,i_sz) = (x.^2 + y.^2) < p_sz_i^2;
end
...
%Pre-allocate output
mean_width_map = zeros(rows, cols, 4);
mean_ori_map = zeros(rows, cols, 4);
mean_curv_map = zeros(rows, cols, 4);
...
for i_pt = 1:length(potential_idx)
    ...
    for i_sz = 1:4
        all_vessels_patch_sz = all_vessels_patch & circ_mask(:,:,i_sz);
        mean_width_map(r_i-pad_sz, c_i-pad_sz,i_sz) = mean(width_patch(all_vessels_patch_sz));
        mean_curv_map(r_i-pad_sz, c_i-pad_sz,i_sz) = mean(curvature_patch(all_vessels_patch_sz));
        mean_ori_map(r_i-pad_sz, c_i-pad_sz,i_sz) = mean(orientation_patch(all_vessels_patch_sz));
    end
