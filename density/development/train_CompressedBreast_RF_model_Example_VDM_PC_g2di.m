...
training_labels((train_size*number_of_samples+1):end)=2;
        ...
        intcounter=0; 
            for i=1:length(ListOfCancerImages)
                if ~isempty(strfind(ListOfCancerImages{i},['-VDM-g2di_Example_nos_' int2str(number_of_samples) '_init_' int2str(init) '.mat'])) 
...
%                     new_cancer_fold_indx(intcounter)=i; 
            ...
            for i=1:length(ListOfControlImages)
                if ~isempty(strfind(ListOfControlImages{i},['-VDM-g2di_Example_nos_' int2str(number_of_samples) '_init_' int2str(init) '.mat'])) 
                    intcounter=intcounter+1; 
