function [density_percents] = ...
    read_density_form_batch(form_name, num_forms, excel_file, reader_initials, debug_flag)
...
%min_profiles = zeros(num_forms, 4);
...
%     min_profiles(ii, :) = min(debug_out.profile_sum);
    nhs_ids(ii) = nhs_id;
    dobs{ii} = dob;
...
%         xlswrite(excel_file, {'RCC'}, 1, 'K1');
%         xlswrite(excel_file, {'RMLO'}, 1, 'L1');
%         xlswrite(excel_file, {'LCC'}, 1, 'M1');
%         xlswrite(excel_file, {'LMLO'}, 1, 'N1');
    end
...
%     xlswrite(excel_file, min_profiles, 1, ['K', start_row]);
end
