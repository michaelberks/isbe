...
metadata.(dicom_name_lookup('0028', '0004')) = getPhotometricInterp(X, map, txfr, metadata);
...
function pInt = getPhotometricInterp(X, map, txfr, metadata)
...
%         if isfield(metadata,'PhotometricInterpretation')
%             pInt=metadata.PhotometricInterpretation;
%         end
        if ~strcmp(metadata.PhotometricInterpretation,'')
            pInt=metadata.PhotometricInterpretation;
        else
            pInt = 'MONOCHROME2';
        end
