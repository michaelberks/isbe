function failures = stepwedge_from_saved(varargin)
...
for i_file=1:n_process
    ...
    if do_errorcheck(density_data, mlo)
        ...
        try
            ...
            %make marker mask
            [marker_mask] = make_marker_mask(IMAGE,...
                density_data.x_marker, density_data.y_marker, density_data.r_marker, resize_factor);
            ...
            if left_breast
                ...
                marker_mask = rot90(marker_mask,2); %don't forget the other masks!
            end
            ...
            %If MLO mammogram compute pectoral mask from saved annotation
            if mlo
            ...
            calibration_data_filename = 'C:\isbe\density\stepwedge calibration\test_calibration_array';
            calibration_data = u_load(calibration_data_filename);
            
            %Can now produce gland_thickness
            gland_thickness = sw_vs_gland_thick(sw_lookup(IMAGE+1), breast_thickness, calibration_data);
