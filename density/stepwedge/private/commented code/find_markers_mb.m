function [markers, region] = find_markers_mb( mammo , marker_template, reduction_factor, left_breast, plot_results, debug)
...
edge_mask = bwselect(mammo/256 > white_thresh, edge_c, edge_r, 4);
...
for i_r = 1:R %two region definitions
    ...
    [region_lims] = region_definitions_mb([rows cols], i_r, large_image, left_breast, padding); %Define regions
    ...
    for i_m = 1:markers_to_find
        ...
        %[g2d_response,~,~,n2d_response] = gaussian_2nd_derivative_line(mammo_i, 10);
        %g2d_mask = (g2d_response < -1000) & (n2d_response < -1000);
    ...
    if any(markers_raw(:,1)) %if markers have been found
...
if plot_results
    ...
    [region_lims] = region_definitions_mb([rows cols], region, large_image, left_breast, padding);
