function [density_percent nhs_id, dob, yes, debug_out] = ...
    read_TPdensity_form(image_in, debug_flag)
...
%start and end points for the 8 VAS lines
num_lines = 8;
line_scale_orig = zeros(2*num_lines, 2);
...
for jj = 1:2
    for ii = 1:4
        ...
        for xx = 1:2
            idx = 8*(jj-1) + 2*(ii-1) + xx;
            line_scale_orig(idx,:) = [x(xx) y];
...
%Points in yes box
[yes_x yes_y] = meshgrid(302:323, 727:748);
[no_x no_y] = meshgrid(402:423, 727:748);
...
% the image
line_scales_image = zeros(2*num_lines, 2, 8);
for ii = 1:4
    offset = 2*(ii+1);
    
    %Compute the start and end points in the original co-ordinate frame
    ls = line_scale_orig + repmat([0 offset], 2*num_lines, 1);
...
profile_sum = zeros(400,num_lines);
profile_min = zeros(num_lines,1);
density_percent = zeros(num_lines,1);
...
debug_out.error = 0;
debug_out.no_mark = zeros(num_lines,1);
for ll = 1:num_lines   
    ...
    for ii = 1:8     
        ...
        if debug_flag == 2
            ...
            figure(prof_fig); subplot(num_lines, 1, ll); hold on;
...
% Read the question
%YES box
yes_affine = transform_points_affine([yes_x(:)';yes_y(:)'], T);

yes_x_affine = reshape(yes_affine(1,:), size(yes_x));
yes_y_affine = reshape(yes_affine(2,:), size(yes_y));



%Compute the barcode points with respect to the large image
yes_x_affine_large = yes_x_affine / rescale;
yes_y_affine_large = yes_y_affine / rescale;

%Now take a small region from the image to speed up interpolation
sc = floor(min(yes_x_affine_large(:))) - 1;
ec = ceil(max(yes_x_affine_large(:))) + 1;
sr = floor(min(yes_y_affine_large(:))) - 1;
er = ceil(max(yes_y_affine_large(:))) + 1;

yes_region = image_in(sr:er, sc:ec);
yes_x_affine_large = yes_x_affine_large - sc + 1;
yes_y_affine_large = yes_y_affine_large - sr + 1;

yes_region_i = interp2(yes_region, yes_x_affine_large, yes_y_affine_large, 'nearest');
yes_mean = mean(1-yes_region_i(:));

%NO box
no_affine = transform_points_affine([no_x(:)';no_y(:)'], T);

no_x_affine = reshape(no_affine(1,:), size(no_x));
no_y_affine = reshape(no_affine(2,:), size(no_y));

%Compute the barcode points with respect to the large image
no_x_affine_large = no_x_affine / rescale;
no_y_affine_large = no_y_affine / rescale;

%Now take a small region from the image to speed up interpolation
sc = floor(min(no_x_affine_large(:))) - 1;
ec = ceil(max(no_x_affine_large(:))) + 1;
sr = floor(min(no_y_affine_large(:))) - 1;
er = ceil(max(no_y_affine_large(:))) + 1;

no_region = image_in(sr:er, sc:ec);
no_x_affine_large = no_x_affine_large - sc + 1;
no_y_affine_large = no_y_affine_large - sr + 1;

no_region_i = interp2(no_region, no_x_affine_large, no_y_affine_large, 'nearest');
no_mean = mean(1-no_region_i(:));

debug_out.yes_mean = yes_mean;
debug_out.no_mean = no_mean;
debug_out.no_yn = (yes_mean < 0.01) && (no_mean < 0.01);

yes = yes_mean > no_mean;
...
if debug_flag
    figure(im_fig);
    plot(yes_x_affine, yes_y_affine, 'g.');
    plot(no_x_affine, no_y_affine, 'r.');
    
    figure; 
    subplot(1,2,1); imagesc(yes_region_i); axis image; colormap(gray(256)); hold on;
    subplot(1,2,2); imagesc(no_region_i); axis image; colormap(gray(256)); hold on;
end
