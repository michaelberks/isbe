function [density_percents] = ...
    read_TPdensity_form_batch(form_name, num_forms, excel_file, reader_initials, debug_flag)
...
for ii = 1:num_forms
    ...
    [dp nhs_id dob yes debug_out] = read_TPdensity_form(form, debug_flag);
    ...
    if debug_out.barcode_warning
        display(['Warning in form ', num2str(ii)]);
        warnings{ii} = [warnings{ii} 'Uncertainty in barcode reading, check NHS No. and DoB; '];
    end
    ...
    if any(debug_out.no_mark)
        ...
        if debug_out.no_mark(1)
            warnings{ii} = [warnings{ii}, ' RML (2);'];
        end
        if debug_out.no_mark(2)
            warnings{ii} = [warnings{ii}, ' LML (2);'];
        end
        if debug_out.no_mark(3)
            warnings{ii} = [warnings{ii}, ' RCC (2);'];
        end
        if debug_out.no_mark(4)
            warnings{ii} = [warnings{ii}, ' LCC (2);'];
        end
        if debug_out.no_mark(5)
            warnings{ii} = [warnings{ii}, ' RML (1);'];
        end
        if debug_out.no_mark(6)
            warnings{ii} = [warnings{ii}, ' LML (1);'];
        end
        if debug_out.no_mark(7)
            warnings{ii} = [warnings{ii}, ' RCC (1);'];
        end
        if debug_out.no_mark(8)
            warnings{ii} = [warnings{ii}, ' LCC (1);'];
        end
    end
    %Check yes/no result
    if debug_out.no_yn
        warnings{ii} = [warnings{ii}, ' Reduction Yes/No missing;'];
        yes_no{ii} = '-';
    elseif yes
        yes_no{ii} = 'y';
    else
    ...
    if yes
        yn_means(ii) = debug_out.yes_mean;
    else
        yn_means(ii) = debug_out.no_mean;
    end
    
    %Write out the form to PDF
    ...
    if form_ratio > a4_ratio
        nx = a4_ratio / form_ratio;
        ny = 1;
    else
        nx = 1;
        ny = form_ratio / a4_ratio;
    end

    %Create axes to position form in figure
    a1 = axes(...
        'Units', 'normalized',...
        'position', [(1-nx)/2, (1-ny)/2, nx, ny]);
    ...
    %Print to pdf
    form_name = [form_dir num2str(nhs_id, 10) '_' reader_initials '.pdf'];
...
if ~isempty(excel_file);
    ...
    if headings
        ...
        xlswrite(excel_file, {'RML (Current)'}, 1, 'C1');
        xlswrite(excel_file, {'LML (Current)'}, 1, 'D1');
        xlswrite(excel_file, {'RCC (Current)'}, 1, 'E1');
        xlswrite(excel_file, {'LCC (Current)'}, 1, 'F1');
        xlswrite(excel_file, {'RML (Previous)'}, 1, 'G1');
        xlswrite(excel_file, {'LML (Previous)'}, 1, 'H1');
        xlswrite(excel_file, {'RCC (Previous)'}, 1, 'I1');
        xlswrite(excel_file, {'LCC (Previous)'}, 1, 'J1');
        xlswrite(excel_file, {'>10% reduction'}, 1, 'K1');
        xlswrite(excel_file, {'Reader initials'}, 1, 'L1');
        xlswrite(excel_file, {'Warnings'}, 1, 'M1');
        xlswrite(excel_file, {'Time read'}, 1, 'N1');
        xlswrite(excel_file, {'Y/N Score'}, 1, 'O1');
    end
    ...
    xlswrite(excel_file, yes_no, 1, ['K', start_row]);
    xlswrite(excel_file, reader_initials_all, 1, ['L', start_row]);
    xlswrite(excel_file, warnings, 1, ['M', start_row]);
    xlswrite(excel_file, timings, 1, ['N', start_row]);
    xlswrite(excel_file, yn_means, 1, ['O', start_row]);
