function make_TPform_pdf_batch(xls_file, form_folder, sheet)
...
for pp = 2:size(patient_data,1)
    ...
    try
        ...
        %Take the current appointment date from the 5th column
        app_date_curr = patient_data{pp,5};
        if valid_date(app_date_curr)
        ...
        %Take the previous appointment date from the 6th column
        app_date_prev = patient_data{pp,6};
        if valid_date(app_date_prev)
        ...
        display(['Name: ' full_name ', ID: ' num2str(nhs_id) ' DOB: ' dob ' App date (prev): ' app_date_prev ' (curr): ' app_date_curr]);
        make_TPform_pdf(full_name, second_name, nhs_id, dob, sx_id, app_date_curr, app_date_prev, form_folder);
    catch
        display(lasterr);
