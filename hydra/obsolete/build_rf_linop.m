function [] = build_rf_linop(varargin)
...
%Set defaults for unix system - environment variables listed in .bashrc (and utils/mb_bash.m)
    ...
    [z num_angles] = unix('echo $NUM_ANGLES'); num_angles = str2num(num_angles); %#ok
...
sampling_method_args.num_angles = args.num_angles;
...
sampling_method_args.win_size = win_size;
sampling_method_args.num_levels = num_levels;
sampling_method_args.num_angles = num_angles;
sampling_method_args.bg_dir = [mberksroot, 'classification/data/smooth512x512_patches/train/'];
sampling_method_args.num_bgs = 10460;
sampling_method_args.bg_zeros = 5;

%set up arguments for main forest
forest_args.sampling_method = 'sample_training_data_linop';
forest_args.sampling_method_args = sampling_method_args;
forest_args.d = round(sqrt(num_levels*num_angles*win_size*win_size));
forest_args.n_trees = n_trees;
forest_args.tree_root = mberksroot;

forest_name = ...
    ['classification/rf/rf_linop_'...
    zerostr(win_size,1) '_' zerostr(num_levels,1) '_' zerostr(num_angles,1)...
    '_job', zerostr(job_idx,2)];

forest_args.tree_dir = [forest_name '_trees/'];
forest_args.save_path = [forest_name '.mat'];
