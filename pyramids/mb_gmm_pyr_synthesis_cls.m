function [synthesised_image, pyramid, cluster_image] = mb_gmm_pyr_synthesis_cls(varargin)
%
...
if ~isempty(args.TargetImage)
    ...
    [pyramid p_sizes] = mb_buildGpyr(args.TargetImage, args.PyrLevels); %
...
for level = args.CutOffLevelCls:-1:1
    ...
    for ori = 1:args.PyrOrientations
        ...
        %set cls map
        syn_args.ClsMap = args.ClsMap{level,1} == ori;

        %Set paths to GMM models
        syn_args.PathToNormalGMM = ...
            [args.ModelDir, args.ModelNameCls, '_', num2str(level), '_', num2str(ori), '_0.mat'];
        syn_args.PathToClsGMM = ...
            [args.ModelDir, args.ModelNameCls, '_', num2str(level), '_', num2str(ori), '_1.mat'];

    
        if exist(syn_args.PathToNormalGMM, 'file') && ...
            exist(syn_args.PathToClsGMM, 'file')
            
            display(['Synthesising from ', syn_args.PathToNormalGMM]);
