...
function resdft = mb_reconSFpyrLevs(pyr,pind,log_rad,Xrcos,Yrcos,angle,nbands,levs,bands);
...
if any(levs > 1)
  ...
  if  (size(pind,1) > lo_ind)
    nresdft = mb_reconSFpyrLevs( pyr(1+sum(prod(pind(1:lo_ind-1,:)')):size(pyr,1)),...
  ...
  resdft = zeros(dims);
  resdft(lostart(1):loend(1),lostart(2):loend(2)) = nresdft .* lomask * 4;
