function [roc_pts auc tp_count fp_count auc_se operating_pts] =...
    calculate_roc_curve_exclusions(class_vals, pos_labels, neg_labels,operating_pts)
...
elseif length(operating_pts) == 1
    operating_pts = linspace(min(class_vals)-eps, max(class_vals), operating_pts);
end

%make sure class_labels are logicals
if ~islogical(pos_labels)
    pos_labels = logical(pos_labels);
end
if ~islogical(neg_labels)
    neg_labels = logical(neg_labels);
end

%compute number of positive/negative labels
n_pos = sum(pos_labels);
n_neg = sum(neg_labels);
...
for ii = 1:n_pts
    ...
    tp_count(ii) = sum(class_vals(pos_labels) > th); %also FN = n_pos - TP;
    ...
    fp_count(ii) = sum(class_vals(neg_labels) > th); %also TN = n_neg - FP;
