function [roc_pts auc tp_count fp_count auc_se] = calculate_roc_image2(...
    prob_map, pos_label, neg_label, operating_pts, varargin)
...
for ii = 1:n_pts
    ...
    if args.thin
        %Thin the map
        thresh_map = bwmorph(thresh_map, 'thin', args.thin);
    end
    if args.dilate
        %Dilate the map
        dilate_map = imdilate(thresh_map, strel('disk', args.dilate));
    else
        dilate_map = thresh_map;
    end
    ...
    tp_count(ii) = sum(dilate_map(:) & pos_label(:)); %also FN = n_pos - TP;
