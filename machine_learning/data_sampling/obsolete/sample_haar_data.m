function [haar_samples] = sample_haar_data(gf1, gf2, rows, cols, varargin)
...
% Call new function
haar_samples = sample_filter_responses(cat(4, gf1, gf2), ...
                              rows, cols, varargin);


%% Old code (for posterity)
function [haar_samples] = old_code(gf1, gf2, rows, cols, varargin)

% Unpack the arguments:
args = u_packargs(varargin, '0',...
    'win_size', 3,...
    'pca', [], ...
	'sigma_range', []); % not used but avoids warnings
...
haar_samples = zeros(num_samples, 2*num_levels*win_size^2);
...
if ~isempty(args.pca)
    ...
    haar_samples = bsxfun(@minus, haar_samples, args.pca.mean)*args.pca.modes;
end
