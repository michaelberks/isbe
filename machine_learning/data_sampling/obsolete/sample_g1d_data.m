function [g1d_samples] = sample_g1d_data(gf1, gf2, rows, cols, varargin)
%SAMPLE_G1D_DATA *Insert a one line summary here*
%   [dt_samples] = sample_g1d_data(dt, rows, cols, rotate)
%
% Inputs:
...
% Call new function
g1d_samples = sample_filter_responses(cat(4, gf1, gf2), ...
                             rows, cols, varargin);

                         
%% Old code (for posterity)
function g1d_samples = old_code(gf1, gf2, rows, cols, varargin)
...
g1d_samples = zeros(num_samples, 2*num_levels*win_size^2);
...
if ~isempty(args.pca)
    ...
    g1d_samples = bsxfun(@minus, g1d_samples, args.pca.mean)*args.pca.modes;
end
...
old_samples = old_code(gf1, gf2, rows, cols);
new_samples = sample_filter_responses(cat(4, gf1, gf2), rows, cols);
