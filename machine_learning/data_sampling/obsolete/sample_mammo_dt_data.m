function [training_data] = sample_mammo_dt_data(varargin)
...
[mammo_names] = get_mammo_info(image_list);
...
for kk = 1:num_images
    ...
    %load in image - flip if right breast
    mammo = u_load([image_dir image_list(im_order(kk)).name]);
    if ~isempty(args.resize)
        mammo = imresize(mammo, args.resize, 'bilinear');
    end
    [row, col] = size(mammo);
    
    %load in mask and resize/invert if necessary
    mask_name = dir([mask_dir, '*' mammo_names{im_order(kk)} '*.mat']);
    mask = u_load([mask_dir mask_name(1).name]);
    if any([row col] ~= size(mask))
        mask = imresize(mask, [row, col]);
    end
    if args.invert_mask
        mask = ~mask;
    end
    ...
    if strcmpi(mammo_names{im_order(kk)}, 'R')
        mammo = fliplr(mammo);
        mask = fliplr(mask);
    end
    ...
    dt = compute_dual_tree(mammo, args.num_levels, args.use_nag);
