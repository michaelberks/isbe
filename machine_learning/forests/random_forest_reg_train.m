function [predictor] = random_forest_reg_train(varargin)
%RANDOM_FOREST_REG_TRAIN given input data and an output variable,
%builds a random forest for regression
%   [random_forest] = random_forest_reg_train(varargin)
%
% RANDOM_FOREST_CLASS_TRAIN uses the U_PACKARGS interface function
...
%
% Example: [predictor] = random_forest_reg_train('X', X, 'y', y);
%
...
%
% See also: TREE_REG_TRAIN RANDOM_FOREST_REG_PREDICT
%
% Created: 13-Oct-2009
...
%**********************************************************************
% IMPORTANT: to compile on the CSF we need to list any of the functions
% used to generate training data here
%
...
%Check start_tree <= num_trees
if start_tree > n_trees
    display(['Warning, a forest with at least ' num2str(n_trees) ' trees has already been built.']);
    return;
end
...
if ~args.quiet; tb = timebar('limit', n_trees); end
for ii = start_tree:n_trees
	if ~args.quiet; timebar(tb,'title', sprintf('Sampling tree #%02i',ii)); end
    ...
    [X y] = feval(args.sampling_args.sampling_method, args);
    t = toc;
    ...
    tree = tree_reg_train(X, y, tree_args); %#ok
    clear X y;
	...
	if ~args.quiet; timebar(tb,'advance'); end
end
...
predictor.sampled_data_dir = tree_args.sampled_data_dir;
