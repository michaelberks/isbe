function [random_forest] = mb_random_forest_two_images_a(varargin)
...
if strcmpi(args.feature_type, 'ilp')
    dt_levels = args.num_levels+1;   
else
    dt_levels = args.num_levels;
end

%Compute interpolation knot points for first image
dt = dtwavexfm2b(args.image1, dt_levels); args = rmfield(args, 'image1');
[interp_args1.knot_mag interp_args1.knot_im interp_args1.knot_re interp_args1.dt_dims] = dt_interp_nag_in(dt);
clear dt;

%Compute interpolation knot points for second image
dt = dtwavexfm2b(args.image2, dt_levels); args = rmfield(args, 'image2');
[interp_args2.knot_mag interp_args2.knot_im interp_args2.knot_re interp_args2.dt_dims] = dt_interp_nag_in(dt);
clear dt2;
...
for ii = 1:n_trees
    ...
    [training_data sample_idx1] = ...
        sample_training_data(interp_args1, args.mask1, args.num_train, sample_args);
    
    %Now expand training data
    [training_data(args.num_train+1:2*args.num_train,:) sample_idx2] = ...
        sample_training_data(interp_args2, args.mask2, args.num_train, sample_args);
    y = [true(args.num_train,1); false(args.num_train,1)];
    ...
    if ii == 1
        ...
        if args.do_test1   
            random_forest.image1_votes1 = zeros(size(args.mask1));
            random_forest.image1_total_votes = zeros(size(args.mask1));
        end
        if args.do_test2   
            random_forest.image2_votes1 = zeros(size(args.mask2));
            random_forest.image2_total_votes = zeros(size(args.mask2));
        end
    ...
    tree = mb_tree_class_train(training_data, y, tree_args);
    clear training_data;
    t = toc;
    ...
    if args.do_test1
        ...
        [random_forest.image1_votes1] =...
            classify_image(interp_args1, tree_mask, tree, random_forest.image1_votes1, sample_args, args.max_test_size, args.use_probs);
    ...
    if args.do_test2
        ...
        [random_forest.image2_votes1] =...
            classify_image(interp_args2, tree_mask, tree, random_forest.image2_votes1, sample_args, args.max_test_size, args.use_probs);
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function [training_data sample_idx] = sample_training_data(interp_args, mask, num_train, sample_args)
...
if strcmpi(feature_type, 'ilp')
    ...
    dt_samples = squeeze(...
        dt_interp_nag_out(...
        interp_args.knot_mag, interp_args.knot_im, interp_args.knot_re, interp_args.dt_dims,...
        rows(:), cols(:)));
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function [votes] = classify_image(interp_args, mask, tree, votes, sample_args, max_size, use_probs)
    ...
    for cp = 1:c_parts
        ...
        if strcmpi(feature_type, 'ilp')
            ...
            dt_samples = dt_interp_nag_out(...
                interp_args.knot_mag, interp_args.knot_im, interp_args.knot_re, interp_args.dt_dims,...
                win_rows(:), win_cols(:)); clear win_rows win_cols;
