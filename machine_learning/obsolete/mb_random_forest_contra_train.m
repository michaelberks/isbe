function [random_forest abnormal_roi_abnormalvotes normal_roi_abnormalvotes abnormal_roi_totalvotes normal_roi_totalvotes] = mb_random_forest_contra_train(varargin)
...
for ii = 1:n_trees
    ...
    if do_err
        [X y row col abnormal_idx normal_idx] = feval(sampling_method, sampling_method_args);
    ...
    if X_test
        ...
        contralateral_pair = u_load(sampling_method_args.pair_name);
        abnormal_roi = contralateral_pair.abnormal_roi;
        normal_roi = contralateral_pair.normal_roi;
        % create a mask to trough away the training pixels
        mask_abnormal = true(row, col);
        mask_abnormal(abnormal_idx) = false;
        
        mask_normal = true(row, col);
        mask_normal(normal_idx) = false;
        
        
        %Get predictions from new tree
        fit_abnormal = tree_predict_image(...
            'image_in', abnormal_roi,...
            'tree', tree,...
            'num_levels', sampling_method_args.num_levels,...
            'mask', mask_abnormal,...
            'win_size', sampling_method_args.win_size,...
            'do_max', sampling_method_args.do_max);
        fit_normal = tree_predict_image(...
            'image_in', normal_roi,...
            'tree', tree,...
            'num_levels', sampling_method_args.num_levels,...
            'mask', mask_abnormal,...
...
%         random_forest.tree_error(ii) = 1 - mean(strcmp(y_tree, y_test));
        ...
        t = toc;
    display(['Time testing one image = ', num2str(t)]);
