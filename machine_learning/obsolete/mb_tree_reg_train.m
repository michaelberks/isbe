function [tree assignednode] = mb_tree_reg_train(X, y, varargin)
...
%Check y is numeric
if ~isnumeric(y)
   error('stats:treefit:BadData','y must be a numeric matrix. For classification use MB_TREE_CLASS_TRAIN');
end
...
%                ------------------   --------------------------
    ...
    otherwise,     error(['Bad value for ''split_criterion'' parameter: ',args.split_criterion])
end
...
%     <--- left ssx --->    <------------ right ssx ------------>
ssx = (Ysplit1.^2) ./ n1 + ((Ycum(end) - Ysplit1).^2) ./ (N - n1);
