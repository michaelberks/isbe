function [y_fit probs error_trace proximities] = mb_random_forest_prob_predict(forest, X, y)
...
%
%   probs:  Votes for each class for each data point (note by dividing by
...
%pre-allocate space for class probs from each tree
probs = zeros(N, nclasses);
...
for ii = 1:n_trees
    ...
    %Get probability predictions for data from this tree
    [y_tree nodes] = mb_tree_predict(tree, X, 1);
    
    %Increment probs for each class
    probs = probs + y_tree;
    
    if nargin > 2
        %[dummy y_idx] = max(probs, [], 2); clear dummy
        %error_trace(ii) = 1 - mean(strcmp(y_vals(y_idx), y));
        
        %*del-me*
        [dummy error_trace(ii)] = calculate_roc_curve(probs(:,2)/ii, y,(-1:100)/100);
...
%Get index to class with maximum probs
[dummy y_idx] = max(probs, [], 2); clear dummy
