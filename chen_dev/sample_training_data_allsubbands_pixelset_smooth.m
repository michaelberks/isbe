function [training_data training_labels parameters] = sample_training_data_allsubbands_pixelset_smooth(varargin)
%
...
while true %reaching num_samples is checked below, loop terminates with a return statement
    ...
    if spicule
        ...
        [bar_image, dummy, label] = create_ellipse_bar_smooth(width/2, contrast, orientation, row, col);
    end
    clear dummy;
    ...
    if curr_sample >= num_samples
        ...
        %         %Convert into magnitude and phase. This is a fast way, but out off momery
