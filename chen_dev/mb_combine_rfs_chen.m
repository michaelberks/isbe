function [random_forest] = mb_combine_rfs_chen(varargin)
...
args = u_packargs(varargin,... % the user's input
    ...
    {'rf_dir', ... 
    'tree_root', ...
    'tree_dir'},...% the mandatory arguments
    'cptree_root',[],...
    'cptree_dir', [],...
    'rf_list', [],...
    'replace_tree_root', [],...
...
%them in
if args.copy_trees
    tree_dir = [args.cptree_root args.cptree_dir];
    ...
    random_forest.tree_dir = args.cptree_dir;
    random_forest.tree_root = args.cptree_root;
...
for ii = 1:n_rfs
    ...
    rf.tree_root = args.tree_root;
    rf.tree_dir = [args.tree_dir, 'rf_centre_', zerostr(ii,2), '_trees\'];

    %if first forest get information and save in main forest
    if ii == 1
        random_forest.D = rf.D;
        random_forest.d = rf.d;
        random_forest.n_classes = rf.n_classes;
    ...
    elseif (random_forest.D ~= rf.D) || (random_forest.n_classes ~= rf.n_classes)
        %Warn user
...
if isempty(save_path)
    ...
    if ~exist(save_path, 'dir')
       ...
       save([save_path, 'random_forest'], 'random_forest');
