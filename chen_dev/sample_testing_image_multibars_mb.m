function [] = sample_testing_image_multibars_mb(varargin)
...
args = u_packargs(varargin,... % the user's input
    ...
    'bg_dir', 'M:\chen\data\smooth512x512_patches\test\',...
    'width_range', [4 16],...
    'orientation_range', [0 180],...
    'contrast_range', [0 8],...
    'max_numlines', 20,...
    'save_path', 'M:\chen\data\testimage_contrast0to8_multibars\',...
...
while curr_image < num_images+1
    ...
    bg_idx = ceil(length(bg_list)*rand); %% 6130 bg pathces
    ...
    for curr_line = 1:num_lines
        ...
        %randomly select centre
        centre_x = ceil(col * rand);
        centre_y = ceil(row * rand);
        
        %make bar
        [bar_image, label, label_centre] = ...
            create_sin_bar(width/2, contrast, orientation, row, col, squash, centre_x, centre_y);
        
        %multibar_image_old = multibar_image;
