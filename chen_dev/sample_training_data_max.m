function [training_data training_labels parameters] = sample_training_data_max(varargin)
...
while true %reaching num_samples is checked below, loop terminates with a return statement
    ...
    max_dt = squeeze(max(dt_inter, [], 3)); clear dt_inter;

    %pad the edges of subbands and label
    pad_dt = padarray(max_dt, [pad_w pad_w], 'replicate'); clear max_dt;
    pad_label = padarray(label, [pad_w pad_w], 'replicate'); clear label;

    %Get indices for this image and convert to row,col
    image_idx = [bar_idx; bg_idx]; clear bar_idx bg_idx;
    ...
    for ii = 1:num_samples_image
        ...
        if curr_sample == num_samples
            ...
            %Convert into magnitude and phase,  This is a fast way, but out
            %off momery
