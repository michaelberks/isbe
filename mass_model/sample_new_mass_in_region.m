...
function [mass] = sample_new_mass_in_region(varargin)
...
if ~isempty(args.ConditionMode)
    new_com(1:args.NumModes) = args.MassModel.B_com(1:args.NumModes,args.ConditionMode);
end
...
%Rotate shape and shift by origin
new_shape = circshift(new_shape, 1-args.Origin) * inv(args.Rotation);

%Translate shape to new centre
...
if num_obj == 1
    ...
    new_shape_tex(new_shape_tex < 0) = 0;
