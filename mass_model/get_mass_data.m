...
function get_mass_data(mass_files, anno_path, mass_path, if_plot)
    ...
    for ii = 1:N
        ...
        temp = load([anno_path, mass_files(ii).name]);
        ...
        annotated_outline = mass.mass_outline;
        ...
        if strcmpi(mass_name(8), 'R')
            annotated_outline(:,1) = cols + 1 - annotated_outline(:,1);
            mass_ROI = fliplr(mass_ROI);
            subtract_ROI = fliplr(subtract_ROI);
            nipple(1) = cols + 1 - nipple(1);
        end
        
        mass_mask = roipoly(rows, cols,...
            annotated_outline(:,1), annotated_outline(:,2));
        
        mass_mask = imdilate(mass_mask, strel('disk', 49));
