...
for edge_angle = 1:20%[3 6 9 13 16 19] %edges aligned at sub-band orientations
    ...
    [dummy, dummy, dummy, amp_pos_edge, ori_pos_edge, phase_pos_edge] =...
        monofilt(pos_edges_128(:,:,edge_angle), 3, 4, 2, 0.65, 0);
    clear dummy;
    [dummy, dummy, dummy, amp_neg_edge, ori_neg_edge, phase_neg_edge] =...
        monofilt(neg_edges_128(:,:,edge_angle), 3, 4, 2, 0.65, 0);
    clear dummy;
    
    f_amp = figure;
    for level = 1:3
        
        phase_coeffs = amp_pos_edge{level} .* exp(i*phase_pos_edge{level});
        ori_coeffs = amp_pos_edge{level} .* exp(i*ori_pos_edge{level});
        figure(f_amp);
        subplot(2,3,level);
        imagesc(amp_pos_edge{level}); axis image; colormap(gray(256)); hold on;
        quiver(1:2:128, 1:2:128, real(phase_coeffs(1:2:128,1:2:128)), -imag(phase_coeffs(1:2:128,1:2:128)), 'r');
        quiver(1:2:128, 1:2:128, real(ori_coeffs(1:2:128,1:2:128)), -imag(ori_coeffs(1:2:128,1:2:128)), 'g');
        
        phase_coeffs = amp_pos_edge{level} .* exp(i*phase_pos_edge{level});
        ori_coeffs = amp_pos_edge{level} .* exp(i*ori_pos_edge{level});
        subplot(2,3,level+3);
        imagesc(amp_neg_edge{level}); axis image; colormap(gray(256)); hold on;
        quiver(1:2:128, 1:2:128, real(phase_coeffs(1:2:128,1:2:128)), -imag(phase_coeffs(1:2:128,1:2:128)), 'r');
        quiver(1:2:128, 1:2:128, real(ori_coeffs(1:2:128,1:2:128)), -imag(ori_coeffs(1:2:128,1:2:128)), 'g');
        %quiver(4:8:128, 4:8:128, real(phase_coeffs(4:8:128,4:8:128)), -imag(phase_coeffs(4:8:128,4:8:128)), 'r');
        %quiver(4:8:128, 4:8:128, real(ori_coeffs(4:8:128,4:8:128)), -imag(ori_coeffs(4:8:128,4:8:128)), 'g');
...
for edge_angle = 1:20%[3 6 9 13 16 19] %edges aligned at sub-band orientations
    ...
    [amp_pos_edge ori_pos_edge] = gabor_filter('ImageIn', pos_edges_128(:,:,edge_angle));
    [amp_neg_edge ori_neg_edge] = gabor_filter('ImageIn', neg_edges_128(:,:,edge_angle));
    
    f_amp = figure;
    for level = 1:1
        
        ori_coeffs = amp_pos_edge .* exp(i*ori_pos_edge);
        figure(f_amp);
        subplot(2,3,level);
        imagesc(amp_pos_edge); axis image; colormap(gray(256)); hold on;
        quiver(1:2:128, 1:2:128, real(ori_coeffs(1:2:128,1:2:128)), -imag(ori_coeffs(1:2:128,1:2:128)), 'g');
        
        ori_coeffs = amp_pos_edge .* exp(i*ori_pos_edge);
        subplot(2,3,level+3);
        imagesc(amp_neg_edge); axis image; colormap(gray(256)); hold on;
        quiver(1:2:128, 1:2:128, real(ori_coeffs(1:2:128,1:2:128)), -imag(ori_coeffs(1:2:128,1:2:128)), 'g');
...
%%
%pos_lines_256 = ones(256); pos_lines_256(128,97:160) = 2;
%neg_lines_256 = 2*ones(256); neg_lines_256(128,97:160) = 1;
