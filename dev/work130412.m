...
for i_ve = 1:length(v_list)
    load([vessel_dir v_list(i_ve).name], 'quality');
...
%%
quick_vessel_segmentation3(vessel_dir, good_selection(56:end));
%%
fov_dir = 'C:\isbe\nailfold\data\rsa_study\training\fov_masks\';
quick_fov_markup(vessel_dir, fov_dir, good_selection(59:end));
%%
...
%     [vessel_mask] = make_mask_from_contour(contour_struc.outer_edge, contour_struc.inner_edge, rows, cols);
%     figure; imgray(vessel_mask);
%     save([vessel_mask_dir v_files(i_v).name(1:end-12) '_v_mask.mat'], 'vessel_mask');
    ...
    save([vessel_image_dir v_files(i_v).name(1:end-12) '.mat'], 'vessel_patch_equalised');
end
%%
vessel_image_dir = 'C:\isbe\nailfold\data\rsa_study\test\images\';
mkdir(vessel_image_dir);
for i_v = 62:100
    vessel_struc = u_load([vessel_dir v_list(good_selection(i_v)).name]);
    ...
    vessel_patch_smoothed = conv2(g', g, vessel_patch, 'same') ./ im_edges;
    vessel_patch_equalised = vessel_patch - vessel_patch_smoothed;
    save([vessel_image_dir v_list(good_selection(i_v)).name], 'vessel_patch_equalised');
end
%%
vessel_image_dir = 'C:\isbe\nailfold\data\rsa_study\test\images\';
vessel_pred_dir = 'C:\isbe\nailfold\data\rsa_study\test\predictions\detection\rf_classification\180706\';
mkdir(vessel_image_dir);
for i_v = 62:100
    vessel_patch = u_load([vessel_image_dir v_list(good_selection(i_v)).name]);
    vessel_pred = u_load([vessel_pred_dir v_list(good_selection(i_v)).name(1:end-4) '_pred.mat']);
    figure;
    subplot(1,2,1); imgray(vessel_pred);
...
for i_ve = 1:length(v_list)
    vessel_struc = load([vessel_dir v_list(i_ve).name]);
    save([vessel_dir v_list(i_ve).name], 'vessel_struc');
end
%%
for i_ve = [3 266 270]
    vessel_struc = u_load([vessel_dir v_list(i_ve).name]);
    vessel_struc.quality = 2;
    save([vessel_dir v_list(i_ve).name], 'vessel_struc');
    delete([vessel_dir v_list(i_ve).name(1:end-4) '_edge.mat']);
end
%%
contour_dir = 'C:\isbe\nailfold\data\rsa_study\vessel_contours\normal\';
vessel_dir = 'C:\isbe\nailfold\data\rsa_study\apexes\normal\';
%%
mkdir(contour_dir);
mkdir([contour_dir 'quarantine']);

v_list = dir([vessel_dir, '*vessel_edge.mat']);
for i_ve = 1:length(v_list)
    if ismember(i_ve, [39 41 44]); 
        movefile([vessel_dir v_list(i_ve).name], [contour_dir 'quarantine\' v_list(i_ve).name]);
    else
        movefile([vessel_dir v_list(i_ve).name], [contour_dir v_list(i_ve).name]);
    end
end
%%
...
%%
rf_args.tree_dir = [rf_dir 'george\trees\'];
rf_args.sampling_args.y = training_y;
rf_args.sampling_args.X = training_x;
predictor = random_forest_reg_train(rf_args);
predictions = random_forest_reg_predict(predictor, test_x, 0);

hold all; plot(sort(abs(predictions(old_n_pts+1:2*old_n_pts)-test_y(old_n_pts+1:2*old_n_pts))), (1:old_n_pts)/old_n_pts);
%%
