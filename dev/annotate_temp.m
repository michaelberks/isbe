function annotate_temp(varargin)
...
initialise_figs;
    ...
    % creates and positions the user controls on the main annotation panel                   
        ...
        handles.mb = uimenu(handles.file_menu,'Label','Load mb border',...
    ...
    % ROI to be annotated
        ...
        handles.fig5 = figure(  'Visible','off',...
                                'Name', 'Mass ROI - no annotation',...
                                'NumberTitle', 'off',...
                                'MenuBar', 'none',...
                                'CloseRequestFcn',@close_fig_Warning);
        handles.axes5 = axes;                    
        set(handles.fig5,'CurrentAxes', handles.axes5);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % --------------------------------------------------------------------
    function mb_Callback(hObject, eventdata)
    % Callback to the "file->open" function used to load the mammogram into
    % the program
        ...
        if filename
            ...
            if (mass_mb.R1 ~= handles.r1)
                mass_mb.mass_outline(:,2) = mass_mb.mass_outline(:,2)...
                    + mass_mb.R1 - handles.r1;
            end
            if (mass_mb.C1 ~= handles.c1)
                mass_mb.mass_outline(:,1) = mass_mb.mass_outline(:,1)...
                    + mass_mb.C1 - handles.c1;
            end
            axes(handles.axes3);
            plot(mass_mb.mass_outline(:,1), mass_mb.mass_outline(:,2),...
                'b:', 'HitTest', 'off');
            axes(handles.axes4);
            plot(mass_mb.mass_outline(:,1), mass_mb.mass_outline(:,2),...
                'b:', 'HitTest', 'off');
    ...
    % the annotations, replacing the existing ones if necessary
            ...
            switch selection,
                case 'Yes',
                    ...
                    if isfield(handles, 'nipple_point')
                        ...
                        handles = rmfield(handles, 'nipple_point');
                    end
        ...
        if filename
            ...
            set(handles.fig5, 'Position',[100,200,360,480],...
                              'Visible', 'on',...
                              'Name', [handles.file_orig_text, ' - no annotation']);
            axes(handles.axes5)
            handles.im5 = imagesc(handles.roi); axis image; hold on;
            colormap(gray(256));
            
            linkaxes([handles.axes3, handles.axes4, handles.axes5]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % from the mass outline
        mass_outline(end, :) = [];
        if isempty(mass_outline)
            delete(plots.p_m3); plots = rmfield(plots, 'p_m3');
            delete(plots.p_m4); plots = rmfield(plots, 'p_m4');              

            delete(plots.l_m3); plots = rmfield(plots, 'l_m3');
            delete(plots.l_m4); plots = rmfield(plots, 'l_m4');

            delete(plots.cp_m3); plots = rmfield(plots, 'cp_m3');
            delete(plots.cp_m4); plots = rmfield(plots, 'cp_m4');
            ...
            case 'Yes',
                ...
                if cs == 1
                    if length(spicules)
                        ...
                        delete(plots.p_s3); plots = rmfield(plots, 'p_s3');
                        delete(plots.p_s4); plots = rmfield(plots, 'p_s4');

                        delete(plots.l_s3); plots = rmfield(plots, 'l_s3');
                        delete(plots.l_s4); plots = rmfield(plots, 'l_s4');

                        delete(plots.cp_s3); plots = rmfield(plots, 'cp_s3');
                        delete(plots.cp_s4); plots = rmfield(plots, 'cp_s4');
