...
for ii = 1:30
    ...
    for level = 1:4
        ...
        [CLS_result] = mb_cls_selection('ImageIn', gp{level+1},...
            'GaborFilterArgs', GaborFilterArgs, 'MinCLSLength', 10*(5-level), 'Connectivity', 0,...
            'Thin', 1, 'Threshold', 0.2);
        subplot(2,2,level);
        imagesc(gp{level+1}); axis image; colormap(gray(256));
        hold on;
        [y_pts x_pts] = find(CLS_result.CLS);
        plot(x_pts, y_pts, 'r.', 'MarkerSize', 4);
        title(['Region = ', num2str(ii), ' Level = ', num2str(level)]);
    end
...
for ii = 1:10
    ...
    for level = 2:2
        ...
        for ori = 1:6
            ...
            plot(x_pts, y_pts, 'r.', 'MarkerSize', 4);
            %title(['Tau = ', num2str(tau(jj)), ' Length = ',
            %num2str(lengths(jj))]); 
