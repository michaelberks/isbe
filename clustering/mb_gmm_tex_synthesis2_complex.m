function [synthesised_image cluster_image] = mb_gmm_tex_synthesis2_complex(varargin)
%
...
synthesised_image(args.PixelOrder) = complex(NaN, NaN);
...
for ii = 1:pixels_to_fill
    ...
    sampled_window = [(reshape(sample_window(synthesised_image, ...
                      ...
                      model.Stats.Mean2) / model.Stats.SD2];
                  
    sampled_window = [real(sampled_window) imag(sampled_window)];
    ...
    sampled_window(isnan(sampled_window)) = (p*model.Stats.SD) + model.Stats.Mean;    
    sampled_window = sampled_window(1:end/2) + i*sampled_window(end/2+1:end);
    ...
    if pixel_wise_synthesis
        ...
        % Update the record of pixel fille
