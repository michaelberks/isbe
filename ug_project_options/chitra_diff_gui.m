...
%--------------------------------------------------------------------------
    function save_user_data
        ...
        %ID
        filename = [user_path, zerostr(user_data.user_id,2), '_3.mat'];
    ...
    function display_welcome_screen
        ...
        if ~isfield(ui, 'main_fig');
            ui.main_fig = figure(...
                ...
                'Visible','on',...
                'Name', 'Synthetic mammogram density study: Reading Model III: Detecting Difference in Density',...
                'NumberTitle', 'off',...
                'MenuBar', 'none',...
                'WindowStyle', 'normal',...
                'Color', color1,...
...
%--------------------------------------------------------------------------
    function display_observer_screen
        ...
        ui.regions_text = uicontrol(...
            ...
            'FontSize', 14,...
            'FontName', 'Arial',...
            'Position', [buff, buff, 250, 0.8*button_h]);
        
        ui.panel = uibuttongroup(...
            'Parent', ui.main_fig,...
            'Units', 'pixels',...
            'Position', [2*buff + 250, buff, 1000, button_h],... %3*buff+850, buff, 200, button_h
            'Visible', 'on',...
            'BackgroundColor', color3,...
            'ForegroundColor', color1,...
            'SelectionChangeFcn', @rating_Callback);
        
        ui.question = uicontrol(...
            'Parent',ui.panel,...
            'String', 'Is the density of ''B'' at least 10% points lower than the density of ''A''?',...
            'Style','text',...
            'Tag','file_open',...
            'BackgroundColor', color3,...
            'ForegroundColor', color1,...
            ...
            'FontName', 'Arial',...
            'Units', 'normalized',...
            'Position', [0.05 0.00, 0.75, 0.8]); %[2*buff+text_w, buff, 600, button_h]
        ...
        ui.next_image = uicontrol(...
            ...
            'Position', [4*buff+1250, buff, button_w, button_h],...
            'BackgroundColor', color1,...
...
% --------------------------------------------------------------------
    ...
    % 
        if curr_blob <= number_of_blobs
            ...
            %Disable next button and blank out the yes/no option
...
%--------------------------------------------------------------------------
    ...
    [];...
    [];...
    'Reading Model III : Detecting Difference in Density';...
    [];...
    ['In this reading model you will view 50 pairs of synthetic mammograms which represent '...
    'the LCC view of a woman at two time points, with the earlier image shown on the left '...
    'and labelled ''A'' and the latter image shown on the right and labelled ''B''. '...
    'You will be required to estimate whether there is at least a 10 percentage point reduction '...
    'in density between the two images, and indicate this by clicking the appropriate box for each pair. '...
    'The time taken for you to view and interpret each set of mammograms will be recorded.']...
            };
...
% --------------------------------------------------------------------
    ...
    % Callback to allow us to continue
        ...
        if ismember(user_id, valid_ids)
            filename = [user_path, zerostr(user_id,2), '_3.mat'];
...
% --------------------------------------------------------------------
    function next_image_Callback(hObject, eventdata) %#ok
        ...
        %record time taken to read image
        user_data.blob_timings(curr_blob) = toc;
...
% --------------------------------------------------------------------
    function rating_Callback(hObject, eventdata) %#ok
        ...
        switch rating
            case ui.yes
                user_data.ratings(curr_blob, 1) = 1;
            case ui.no
                user_data.ratings(curr_blob, 1) = 2;
        end
