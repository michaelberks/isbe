function [density_percent debug_out] = read_chitra_form2(image_in, debug_flag)
...
% the image
line_scales_image = zeros(20, 2, 8);
for ii = 1:4
    offset = 2*(ii+1);
    
    %Compute the start and end points in the original co-ordinate frame
    ls = line_scale_orig + repmat([0 offset], 20, 1);
...
profile_sum = zeros(400,10);
profile_min = zeros(10,1);
density_percent = zeros(10,1);
...
for ll = 1:10   
    ...
    for ii = 1:8     
        ...
        if debug_flag == 2
            ...
            figure(prof_fig); subplot(10, 1, ll); hold on;
