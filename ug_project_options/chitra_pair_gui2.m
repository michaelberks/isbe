...
%--------------------------------------------------------------------------
    function initialise_data
        ...
        rand('twister', 200 + user_data.user_id);
        ...
        fid = fopen('C:\isbe\ug_project_options\2011\chitra\software\missing_pair.txt');
        ...
        extra_idx = setdiff(1:50, missing_ims);
        extra_ims = extra_idx(randperm(50 - num_missing));
...
%--------------------------------------------------------------------------
    function save_user_data
        ...
        %ID
        filename = [user_path, zerostr(user_data.user_id,2), '_2a.mat'];
    ...
    function display_welcome_screen
        ...
        if ~isfield(ui, 'main_fig');
            ui.main_fig = figure(...
                ...
                'Visible','on',...
                'Name', 'Synthetic mammogram density study: Reading Model II: Side by side Assessment',...
                'NumberTitle', 'off',...
                'MenuBar', 'none',...
                'WindowStyle', 'normal',...
                'Color', color1,...
...
%--------------------------------------------------------------------------
    function display_observer_screen
        ...
        axes_x = (screen_size(3) - 3*buff)/2;
        axes_y = axes_x / 0.8;
        
        axes_pos_a = [centre_x - (axes_x + buff/2), 2*buff + button_h, axes_x, axes_y];
        axes_pos_b = [centre_x + buff/2, 2*buff + button_h, axes_x, axes_y];
        ...
        set(ui.axes_main_a,...
            ...
            'Ytick', [],...
            'YDir','reverse',...
            'Xlim', [0.5 axes_pos_a(3)+0.5],...
            'Ylim', [0.5 axes_pos_a(4)+0.5]);
        set(ui.axes_main_b,...
            'Position', axes_pos_b,...
            'Xtick', [],...
            'Ytick', [],...
            'YDir','reverse',...
            'Xlim', [0.5 axes_pos_b(3)+0.5],...
            'Ylim', [0.5 axes_pos_b(4)+0.5]);
...
% --------------------------------------------------------------------
    ...
    % 
        if curr_blob <= number_of_blobs
            ...
            blob_mammogram_a = u_load([blob_path, zerostr(user_data.blob_order(curr_blob),3) '_a.mat']);
            blob_mammogram_b = u_load([blob_path, zerostr(user_data.blob_order(curr_blob),3) '_b.mat']);
            
            %Compute the apsect ratios for these images (they may vary from
            %pair to pair)
            aspect_ratio = size(blob_mammogram_a,1) / size(blob_mammogram_a,2);
            axes_y = axes_pos_a(3)*aspect_ratio;
            ...
            set(ui.region_a,...
                'Visible', 'on',...
                'CData', blob_mammogram_a,...
                'XData', [1 axes_pos_a(3)],...
                'YData', [1 axes_pos_a(4)]);
            
            set(ui.region_b,...
                'Visible', 'on',...
                'CData', blob_mammogram_b,...
                'XData', [1 axes_pos_a(3)],...
                'YData', [1 axes_pos_a(4)]);
            clear blob_mammogram_*;
            
            set(ui.text_a, 'position', [axes_pos_a(3) - 50 50]);
            set(ui.text_b, 'position', [axes_pos_b(3) - 50 50]);

            %Update the number of blobs text
            regions_text = ['Mammograms: ', num2str(curr_blob), ' of ', num2str(number_of_blobs)];
...
%--------------------------------------------------------------------------
    ...
    [];...
    [];...
    'Reading Model II : Side by side Assessment';...
    [];...
    ['In this reading model you will view 50 pairs of synthetic mammograms which represent '...
    'the LCC view of a woman at two time points, with the earlier image shown on the left and labelled '...
    '''A'' and the latter image shown on the right and labelled ''B''. You will be required to estimate the density (%) '...
    'of each synthetic mammogram. Please mark your answers using a biro on the visual analogue '...
    'scales that are provided. On each scale the left end point represents 0% (density) and the '...
    'right end point represents 100% (density). Please mark the density (%) of each image on the '...
    'visual analogue scale.  The time taken for you to view and assess each pair of images will be recorded.']...
            };
...
% --------------------------------------------------------------------
    ...
    % Callback to allow us to continue
        ...
        if ismember(user_id, valid_ids)
            filename = [user_path, zerostr(user_id,2), '_2a.mat'];
