...
DRIVE = 0;
if DRIVE
    retroot = [asymmetryroot 'data\retinograms\DRIVE_clean\test\'];
    im_name = 'test';
else %STARE
    retroot = [asymmetryroot 'data\retinograms\STARE\training\'];
    im_name = 'training';
end

pred_dir = [retroot 'predictions\orientation\analytic\'];
do_mono = 0;
do_g1d = 0;
do_g2d = 0;
do_gabor = 1;
...
for ii = 1:20
    ...
    ret = u_load([retroot 'images\' zerostr(ii,2) '_' im_name '.mat']);
    ret = rgb2gray(ret);
    
    if do_mono
        display(['Computing monogenic response for image ' num2str(ii)]);
        %Compute repsonses for mono and save
        [response_map d ori_map scale_map] = monogenic_multiscale(ret, 2, 2, 2, 0.65);
    ...
    if do_g1d
        ...
        %Compute repsonses for mono and save
        [response_map ori_map scale_map] = gaussian_1st_derivative_gradient(ret, [2 4]);
        ori_map = abs(response_map).*exp(2i*ori_map);
    ...
    if do_g2d
        ...
        %Compute repsonses for mono and save
        [response_map ori_map scale_map] = gaussian_2nd_derivative_line(ret, [2 4]);
        ori_map = abs(response_map).*exp(2i*ori_map);
    ...
    if do_gabor
        ...
        %Compute repsonses for mono and save
        num_angles = 12;
        [responses] = compute_gabor_responses(ret, 4, num_angles);
...
%     decomp_type = decomp_types{i_decomp,1};
%     if decomp_types{i_decomp,2}
%         pred_dir = [retroot 'predictions\orientation\analytic\' decomp_type];
...
%     decomp_type = decomp_types{i_decomp,1};
%     if decomp_types{i_decomp,2}
%         pred_dir = [retroot 'predictions\orientation\analytic\' decomp_type '\orientation'];
...
% 
% %%
% retroot = [asymmetryroot 'data\retinograms\DRIVE_clean\test\predictions\orientation\'];
% 
% mkdir([retroot 'analytic\mono\orientations']);
% mkdir([retroot 'analytic\mono\responses']);
% mkdir([retroot 'analytic\mono\scales']);
% 
% for ii = 1:20
%     %load retinogram and merge RGB channels
...
%     
%     ori_map = u_load([retroot 'orientations\' zerostr(i_ret,2) '_training_ori.mat']);
%     
