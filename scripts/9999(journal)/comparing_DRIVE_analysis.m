        ...
        for win_size = [3]
            ...
            for i_repeat = 1:num_repeats
                ...
                switch output_type{1}
                    case 'orientation'
                        load([exp_dir 'test/' num2str(i_repeat) '/true_labels.mat'], 'true_oris_test');
                        
                        [~, error_stats] = ori_error(true_oris_test, predictions);
                        mae(i_repeat) = error_stats.abs_median;
                    case 'width'
                        load([exp_dir 'test/' num2str(i_repeat) '/true_labels.mat'], 'true_widths_test');
                        width_errors = abs(true_widths_test - predictions);
...
%%
% Gaussian filtering at more scales and angles
%
for exp_name = {'angles', 'scales', 'all', 'orig'}
    for win_size = [1 3]
        mae = nan(num_repeats,1);
        for i_repeat = 1:num_repeats
            results_dir = [exp_dir 'results/' num2str(i_repeat) '/orientation/gh2d_all/' exp_name{1} '/' num2str(win_size) '/'];
            try
                load([exp_dir 'test/' num2str(i_repeat) '/true_labels.mat'], 'true_oris_test');
                load([results_dir 'results.mat'], 'predictions');
            catch
...
for i_decomp = 3;
    ...
    for feature_type = {'all' 'real_imag', 'conj', 'mag', 'phase', 'real', 'imag', 'real_abs_imag'}
        auc = nan(num_repeats,1);
        for i_repeat = 1:num_repeats
            results_dir = [exp_dir '/results/' num2str(i_repeat) '/detection/' d_args{i_decomp}.decomp_type{1} '/feature_types/' feature_type{1} '/'];
...
%--------------------------------------------------------------------------
% A)
% Adding more scales and orientations for Gabor filters
for exp_name = {'angles', 'scales', 'all', 'orig'}
    for win_size = [1 3]
        auc = nan(num_repeats,1);
        for i_repeat = 1:num_repeats
...
%--------------------------------------------------------------------------
% B)
% Adding more scales and orientations for Gaussian filters
for exp_name = {'angles', 'scales'}%, 'all', 'orig'}
    for win_size = 3%[1 3]
        auc = nan(num_repeats,1);
        for i_repeat = 1:num_repeats
            results_dir = [exp_dir '/results/' num2str(i_repeat) '/detection/gh2d_all/' exp_name{1} '/' num2str(win_size) '/'];
...
%%
% 8) Varying proportions of Fg and BG
for bg_prop = (2:8)/10
    auc = nan(num_repeats,1);
    for i_repeat = 1:num_repeats
        results_dir = [exp_dir '/results/' num2str(i_repeat) '/detection/g2d/bg_prob/' num2str(bg_prop) '/'];
