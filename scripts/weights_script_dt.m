...
%
% Load model
load C:\isbe\dev\misc\anno_bg\model_dt_sub_masses.mat
%
...
save C:\isbe\dev\mass_model\weights2\weights_sd weights_sd
...
weights_var = [ k_shape / sum(mass_model.L_shape)...
                ...
                mass_model.L_scale;
save C:\isbe\dev\mass_model\weights2\weights_var weights_var
...
r_idx = randsample(101, 101);
save C:\isbe\dev\mass_model\weights2\shape_modes\r_idx r_idx;

shape_offsets = [-15 -10 -5 5 10 15];
scale_offsets = [-.015 -.0125 -.01 -.0075 -.005 -.0025...
    .0025 .005 .0075 .01 .0125 .015];
save C:\isbe\dev\mass_model\weights2\offsets shape_offsets scale_offsets;

weights_fd_shape = zeros(k_shape, 1);
RMS_shapes = zeros(50, 6, k_shape);
%%
for ii = 2:k_shape
    r_name = ['C:\isbe\dev\mass_model\weights2\shape_modes\s_mode', zerostr(ii, 2)];
    [RMS_shape] = ...
    calculate_weights2(u_files1, shape_offsets, [], 'shape_mode', ii,...
    'file_out', r_name, 'indices', r_idx,...
    'model', 'C:\isbe\dev\misc\anno_bg\model_dt_sub_masses.mat');
    weights_fd_shape(ii) = mean(mean(RMS_shape) ./ abs(shape_offsets));
    RMS_shapes(:,:,ii) = RMS_shape;
end
save C:\isbe\dev\mass_model\weights2\RMS_shapes RMS_shapes;
%%
r_name = 'C:\isbe\dev\mass_model\weights2\scale_mode';
[dummy, RMS_scale] = ...
...
save C:\isbe\dev\mass_model\weights2\weights_fd weights_fd*
%%
% 4.
% Compute optimised weights
cd C:\isbe\dev\mass_model\weights2
...
save C:\isbe\dev\mass_model\weights2\weights_opt1 idx_opt1 wso1 weights_opt1 output1;

idx_opt2 = randsample(1:101, 25);
wso2 = [1e-3 0.2e-3];
[weights_opt2, output2] =...
    optimise_weights(mass_model, u_files1, wso2, 'weights_opt', idx_opt2);
save C:\isbe\dev\mass_model\weights2\weights_opt2 idx_opt2 wso2 weights_opt2 output2;
%%
idx_opt3 = randsample(1:101, 25);
wso3 = [1e-3 0.8e-3];
[weights_opt3, output3] =...
    optimise_weights(mass_model, u_files1, wso3, 'weights_opt3', idx_opt3);
save C:\isbe\dev\mass_model\weights2\weights_opt3 idx_opt3 wso3 weights_opt3 output3;

idx_opt4 = randsample(1:101, 25);
wso4 = [4e-3 0.8e-3];
[weights_opt4, output4] =...
    optimise_weights(mass_model, u_files1, wso4, 'weights_opt4', idx_opt4);
save C:\isbe\dev\mass_model\weights2\weights_opt4 idx_opt4 wso4 weights_opt4 output4;
...
% Load weights
%cd C:\isbe\dev\weights
load C:\isbe\dev\mass_model\weights2\weights_var
load C:\isbe\dev\mass_model\weights2\weights_sd
load C:\isbe\dev\mass_model\weights2\weights_fd;
% load weights_opt;
load C:\isbe\dev\misc\anno_bg\model_dt_sub_masses.mat
load C:\isbe\dev\files\u_files.mat
...
save C:\isbe\dev\mass_model\weights2\er_fd_full er_fd_full
...
for ii = 2:n_s_pts
    for jj = 2:n_t_pts
        ...
        er_name = ['C:\isbe\dev\weights2\map\er',...
...
saveas(gcf, 'C:\isbe\dev\weights2\figures\shape_displace1.fig');
...
saveas(gcf, 'C:\isbe\dev\weights2\figures\shape_displace2.fig');

%%
load C:\isbe\dev\weights2\scale_mode.mat
...
saveas(gcf, 'C:\isbe\dev\weights2\figures\scale_displace.fig');
...
ylabel('Texture displacement');
set(gca, 'XTick', [scale_offsets(1:end/2), 0 , scale_offsets(end/2+1:end)]);
saveas(gcf, 'C:\isbe\dev\weights2\figures\scale_displace_mean.fig');
%
...
ylabel('Texture displacement');
set(gca, 'XTick', [scale_offsets(1:end/2), 0 , scale_offsets(end/2+1:end)]);
saveas(gcf, 'C:\isbe\dev\weights2\figures\scale_displace_both.fig');
%%
%%%%
...
save('C:\isbe\dev\weights2\map\er1101', 'er');
