...
ncm_base_iih_patch_extractor::ncm_base_iih_patch_extractor ()
{ 
	//Set smoothing width
	sigma_ = 1;
  ...
  box_.xl = 20; //left
  box_.xr = 20; //right
	box_.spacing = 1.5*sigma_;
}

void ncm_base_iih_patch_extractor::updateBox(
	const vgl_point_2d<double>& centre,
	const vgl_vector_2d<double>& axis)
{
	//Update count of points along x and y directions
	box_.nx = unsigned( ( box_.xl + box_.xr + box_.spacing - 1e-3) / box_.spacing);
	box_.ny = unsigned( ( box_.yu + box_.yd + box_.spacing - 1e-3) / box_.spacing);

	//Get normal to axis vector
  vgl_vector_2d<double> norm(-axis.y(), axis.x());
  
	//Set bottom left coordinates of box
  vgl_vector_2d<double> bottom_left_offset = -box_.xl*axis - box_.yu*norm;
  box_.bottom_left_xy = centre + bottom_left_offset;

	//Set du and dv vectors in box
	box_.du = box_.spacing*axis;
  box_.dv = box_.spacing*norm;

	//vcl_cout << "centre of box: " << centre << vcl_endl;
	//vcl_cout << "bottom left of box: " << box_.bottom_left_xy << vcl_endl;
}

void ncm_base_iih_patch_extractor::extractPatch(vnl_vector<double > &patch_vector)
{
...
void ncm_base_iih_patch_extractor::setImage(const vimt_image_2d_of<vxl_byte>& image)
{
...
vil_structuring_element  ncm_base_iih_patch_extractor::convto_vil_structuring_element(
...
void ncm_base_iih_patch_extractor::readBox(const vcl_string& filename)
{
    ...
    box_ = b;
    vcl_cout << "have read in box location in ncm_base_iih_patch_extractor::readBox" << vcl_endl;
    vcl_cout<<"Box coord " << x << "\t" << y << "\t" <<b.nx<< "\t" << b.ny << vcl_endl;
}

void ncm_base_iih_patch_extractor::writeBox(const vcl_string& filename)
{
