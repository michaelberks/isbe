...
const double ncm_qmarkup_preferences::version_ = 1.5;

//
//  Define class member functions

//: Constructor
ncm_qmarkup_preferences::ncm_qmarkup_preferences()
...
double ncm_qmarkup_preferences::version() const
{
  return version_;
}

ncm_qmarkup_preferences::ImageZoom ncm_qmarkup_preferences::label_image_zoom() const
{
  return label_image_zoom_;
}
ncm_qmarkup_preferences::ImageZoom ncm_qmarkup_preferences::add_vessels_zoom() const
{
  return add_vessels_zoom_;
}
ncm_qmarkup_preferences::ImageZoom ncm_qmarkup_preferences::set_vessel_props_zoom() const
{
  return set_vessel_props_zoom_;
}
ncm_qmarkup_preferences::ImageZoom ncm_qmarkup_preferences::add_haemorrhages_zoom() const
{
  return add_haemorrhages_zoom_;
}

int ncm_qmarkup_preferences::normal_vessel_zoom() const
{
  return normal_vessel_zoom_;
}
int ncm_qmarkup_preferences::enlarged_zoom_relative() const
{
  return enlarged_zoom_relative_;
}
int ncm_qmarkup_preferences::giant_zoom_relative() const
{
  return giant_zoom_relative_;
}
double ncm_qmarkup_preferences::vessel_path_yshift() const
{
  return vessel_path_yshift_;
}

void ncm_qmarkup_preferences::set_use_remote_server(
    bool use_remote_server /* = true */)
{
  use_remote_server_ = use_remote_server;
}

bool ncm_qmarkup_preferences::use_remote_server()
{
...
void ncm_qmarkup_preferences::write_to_registry()
{
...
void ncm_qmarkup_preferences::read_from_registry()
{
  ncm_registry_accessor ra;
  ra.get_current_user_key();
  ...
  if (prefs_version == 1.3)
  {
    ...
    // Need to cast from int to ncm_qmarkup_preferences::ImageZoom explicitly
    int zoom_type;
    ra.read_numeric("LabelImageZoom", zoom_type);
    label_image_zoom_ = 
        static_cast<ncm_qmarkup_preferences::ImageZoom>(zoom_type);
...
//
//: User properties
void ncm_qmarkup_preferences::set_username(const vcl_string& username)
{
  username_ = username;
}
vcl_string ncm_qmarkup_preferences::username() const
{
  return username_;
}
void ncm_qmarkup_preferences::set_advanced_user(
    bool is_advanced_user /* = true */)
{
  is_advanced_user_ = is_advanced_user;
}
bool ncm_qmarkup_preferences::is_advanced_user() const
{
  return is_advanced_user_;
}

vcl_string ncm_qmarkup_preferences::system_username() const
{
...
void ncm_qmarkup_preferences::set_grid_pixels_per_mm(double grid_pixels_per_mm)
{
  grid_pixels_per_mm_ = grid_pixels_per_mm;
}
double ncm_qmarkup_preferences::grid_pixels_per_mm() const
{
  return grid_pixels_per_mm_;
}

void ncm_qmarkup_preferences::set_grid_spacing_mm(double grid_spacing_mm)
{
  grid_spacing_mm_ = grid_spacing_mm;
}
double ncm_qmarkup_preferences::grid_spacing_mm() const
{
