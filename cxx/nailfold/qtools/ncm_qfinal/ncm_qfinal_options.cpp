...
//
//: Constructor
ncm_qfinal_options::ncm_qfinal_options(ncm_qfinal_preferences& preferences,
...
//
//: Destructor
ncm_qfinal_options::~ncm_qfinal_options()
{
}
...
void ncm_qfinal_options::accept()
{
  // save new settings
  write_values();
...
void ncm_qfinal_options::on_apexColorButton_clicked()
{
  const QRgb new_rgb = QColorDialog::getColor().rgb();
  apply_colour_to(ui.apexColorButton, new_rgb);
}
void ncm_qfinal_options::on_vessel_colour_clicked()
{
  const QRgb new_rgb = QColorDialog::getColor().rgb();
  apply_colour_to(ui.vessel_colour, new_rgb);
}
void ncm_qfinal_options::on_vessel_colour_selected_clicked()
{
  const QRgb new_rgb = QColorDialog::getColor().rgb();
...
void ncm_qfinal_options::on_gridPixelsEdit_textEdited(const QString& text)
{
  updateGridLabels();
}
void ncm_qfinal_options::on_gridMillimetresEdit_textEdited(const QString& text)
{
  updateGridLabels();
}
...
void ncm_qfinal_options::apply_colour_to(QPushButton* const button, 
                                          QRgb const& rgb)
{
...
QRgb ncm_qfinal_options::get_colour_from(QPushButton const* const button)
{
  QPixmap pixmap = button->icon().pixmap(16,16);
  return pixmap.toImage().pixel(0,0);
}

void ncm_qfinal_options::updateGridLabels()
{
...
void ncm_qfinal_options::read_values()
{
  // alias for convenience
  ncm_qfinal_preferences& cp = current_preferences_;
...
void ncm_qfinal_options::write_values()
{
  ...
  // Zoom options
  cp.label_image_zoom_ = static_cast<ncm_qfinal_preferences::ImageZoom>
      (ui.labelImageZoom->currentIndex());
  cp.add_vessels_zoom_ = static_cast<ncm_qfinal_preferences::ImageZoom>
      (ui.addVesselsZoom->currentIndex());
  cp.set_vessel_props_zoom_ = static_cast<ncm_qfinal_preferences::ImageZoom>
      (ui.setVesselPropsZoom->currentIndex());
  cp.add_haemorrhages_zoom_ = static_cast<ncm_qfinal_preferences::ImageZoom>
