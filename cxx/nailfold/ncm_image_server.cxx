...
ncm_file_server::ncm_file_server(vcl_string root /* = "." */,
...
int ncm_file_server::read_site_name()
{
...
vcl_string ncm_file_server::site_name() const
{
...
vcl_string ncm_file_server::remote_imagelist_filename() const
{
  return site_name() + "_images.txt";
}
vcl_string ncm_file_server::local_imagelist_filename() const
{
  return root_ + "/" + remote_imagelist_filename();
}

void ncm_file_server::set_root(vcl_string root)
{
...
void ncm_file_server::set_user(vcl_string user)
{
  // Get rid of trailing slashes etc
  user_ = vul_expand_path(user);
}

void ncm_file_server::set_image_dir(vcl_string image_dir)
{
  // Get rid of trailing slashes etc
...
void ncm_file_server::set_markup_dir(vcl_string markup_dir)
{
  // Get rid of trailing slashes etc
  markup_dir_ = vul_expand_path(markup_dir);
	parse_images();
}

void ncm_file_server::set_inbox(vcl_string inbox)
{
  inbox_ = vul_expand_path(inbox);
  parse_inbox();
}

void ncm_file_server::set_outbox(vcl_string outbox)
{
  outbox_ = vul_expand_path(outbox);
}

void ncm_file_server::set_sent(vcl_string sent)
{
  sent_ = vul_expand_path(sent);
}

void ncm_file_server::parse_inbox()
{
...
void ncm_file_server::parse_images()
{
  vcl_vector<vcl_string> extensions;
  extensions.push_back("jpg");
  extensions.push_back("png");
  ...
  for (unsigned i = 0; i < extensions.size(); ++i)
  {
    vcl_string search_string = image_dir_ + "/*." + extensions[i];
    for (vul_file_iterator fn = search_string; fn; ++fn) 
    {
			
      const vcl_string markup_filename = markup_dir_ + "/" + site_name_ + "/" +
          vul_file::strip_directory(vul_file::strip_extension(fn())) + "_markup.txt";
...
//                     image list)
int ncm_file_server::pull_imagelist_from(const ncm_server_handler& server_handler)
{
...
//    n_images >= 0 => download n_images images only (using the existing 
//                     image list)
int ncm_file_server::pull_images_from(const ncm_server_handler& server_handler,
                                      int n_images /* = -1 */)
{
...
int ncm_file_server::pull_markup_from(const ncm_server_handler& server_handler)
{
...
bool ncm_file_server::copy_to_outbox(vcl_string src,
                                     vcl_string dest /* = "" */)
{
...
//  to the Sent folder.
int ncm_file_server::push_outbox_to(const ncm_server_handler& server_handler)
{
...
void ncm_file_server::use_unmarked_only(bool unmarked_only /* = true */)
{
...
bool ncm_file_server::is_using_unmarked_only() const
{
...
unsigned ncm_file_server::n_images() const
{
  return images_.size();
}
...
unsigned ncm_file_server::n_unmarked() const
{
...
unsigned ncm_file_server::n_valid() const
{
  unsigned n_valid = 0;
...
unsigned ncm_file_server::count() const
{
  return n_valid();
}
...
bool ncm_file_server::is_empty() const
{
  return (count() == 0);
}
...
vcl_vector<vcl_string> ncm_file_server::valid_images()
{
...
vcl_string ncm_file_server::current_image()
{
...
bool ncm_file_server::set_current(vcl_string filename)
{
...
void ncm_file_server::set_current_marked()
{
  if (image_iterator_ != images_.end())
...
bool ncm_file_server::is_current_marked() const
{
...
bool ncm_file_server::copy_file(const vcl_string& src,
...
bool ncm_file_server::move_file(const vcl_string& src,
                                const vcl_string& dest) const
{
  vcl_string src_quoted = "\"" + dos_filename(src) + "\"";
  vcl_string dest_quoted = "\"" + dos_filename(dest) + "\"";
...
vcl_string ncm_file_server::dos_filename(const vcl_string& filename) const
{
...
bool ncm_file_server::is_valid(const ncm_marked_image& marked_image) const
{
...
vcl_vector<ncm_marked_image>::iterator ncm_file_server::find_next(
...
//  Return images_.end() if none are valid
vcl_vector<ncm_marked_image>::iterator ncm_file_server::find_previous(
    const vcl_vector<ncm_marked_image>::iterator& initial)
{
...
//  Return images_.end() if none are valid
vcl_vector<ncm_marked_image>::iterator ncm_file_server::find_first()
{
  if (is_empty())
    return images_.end();
...
//  Return images_.end() if none are valid
vcl_vector<ncm_marked_image>::iterator ncm_file_server::find_last()
{
  if (is_empty())
    return images_.end();
  else
